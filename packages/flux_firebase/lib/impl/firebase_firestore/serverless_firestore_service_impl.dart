...
class ServerlessFirestoreServiceImpl extends ServerlessFirestoreService {
  FirebaseApp? _app;
  ServerlessFirestoreServiceImpl({FirebaseApp? app}) {
  ...
  Future<List<Map>> getCategories({
    ...
    return docs.map((doc) {
      ...
      return Map.from({'id': doc.id, ...mapData});
    }).toList();
  }
  ...
  Future<List<Map>> getProducts({
    ...
    if (categoryId != null && categoryId != '0') {
      ...
      query = query.where('CategoryIds', arrayContainsAny: [categoryId]);
    ...
    final list = result
        .map((doc) {
          ...
          return Map.from({...mapData, 'id': doc.id});
        })
        .whereType<Map>()
        .toList();
  ...
  Future<List<String>> getListIdsCategoryByProductId(String productId) async {
    ...
    return categoriesSnapshot.docs
        .map((catDoc) => catDoc.data()['categoryId'].toString())
        .toList();
  }

  @override
  Future<Map?> getCategoryById(String categoryId) async {
    final categoryDoc = await _firestoreCollection.categories
        .doc(categoryId)
        .get();
    ...
    return {...mapData, 'id': categoryDoc.id};
  }
  ...
  Future<Map?> getProduct(String id) async {
    ...
    return {...mapData, 'id': productDoc.id};
  }
  ...
  Future<List<Map>> getOrdersByUserId(
    ...
    final query = _firestoreCollection.orders.where(
      'userId',
      isEqualTo: userId,
    );
  ...
  Future<Map?> updateOrder(
    ...
    final querySnapshot = await _firestoreCollection.orders
    ...
    final orderDoc = await _firestoreCollection.orders
        .where('id', isEqualTo: orderId)
        .get();
