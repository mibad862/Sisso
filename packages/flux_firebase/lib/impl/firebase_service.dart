...
class FirebaseServices extends BaseFirebaseServices {
  ...
  Future<void> init({FluxFirebaseOption? option, String? name}) async {
    ...
    _app = await FirebaseServiceFactory.create<FirebaseCoreService>()
        ?.initializeApp(option: option, name: name);
      ...
      _firebaseAnalytics = FirebaseServiceFactory.createAnalyticsService(app)!
        ..init(
          ...
          adPersonalizationSignalsConsentGranted:
              kFirebaseAnalyticsConfig['adPersonalizationSignalsConsentGranted'],
  ...
  void saveUserToFirestore({User? user}) async {
    // Respect the in-app notification opt-out: don't regenerate an FCM token
    // (calling getToken() would re-provision a token even after a hard opt-out).
  ...
  Widget renderChatScreen({
    ...
    final sender = ChatUser(email: senderEmail, name: senderName);
    final receiver = receiverEmail != null
        ? ChatUser(email: receiverEmail, name: receiverName)
        : null;
  ...
  Future<String?> getMessagingToken() async {
    final appNotification = UserBox().notification;
    if (appNotification?.enable == false) {
      return null;
    }
