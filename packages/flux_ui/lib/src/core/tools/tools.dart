...
class Tools {
  ...
  static void showSnackBar(
    ...
    if (state != null && message != null) {
      state.showSnackBar(SnackBar(content: Text(message), duration: duration));
    }
  }
  ...
  static Future<void> launchMaps({
    ...
    if (address != null && '$address'.trim().isNotEmpty) {
      ...
      final url =
          'www.google.com/maps/search/?api=1&'
  ...
  static Future<void> launchMapsURL(dynamic lat, dynamic long) async {
    ...
    await Tools.launchURL('https://$url', mode: LaunchMode.externalApplication);
  }
  ...
  static Future<bool> launchURL(
    ...
    if (mode == null && needToOpenExternalNonBrowserApp(url)) {
    ...
    if (!isLaunched) {
      // Try again if the previous mode was `externalNonBrowserApplication`
      if (mode == LaunchMode.externalNonBrowserApplication) {
        printError(
          'Could not launch $originUrl with mode ${mode?.name}. Try again!',
        );
        mode = LaunchMode.externalApplication;
        try {
          isLaunched = await launchUrl(
    ...
    if (!isLaunched) {
      // Try again if the previous mode was `externalApplication`
  ...
  /// If this url can open external app, it will redirect to external app
  static bool needToOpenExternalNonBrowserApp(String? url) {
  ...
  /// If this url can open external app/browser, it will redirect to external
  /// app/browser
  static bool needToOpenExternalApp(String? url) {
    ...
    final isNeedToOpenExternalNonBrowserApp = needToOpenExternalNonBrowserApp(
      url,
    );

    if (isNeedToOpenExternalNonBrowserApp) {
      return true;
    }

    for (var app in externalAppURLs) {
      if (url.startsWith('http') && url.isTheSameDomain(app)) {
        return true;
      }
    }
    return false;
  ...
  static String? getCurrencyCode(String? currency) {
    ...
    if (currencies.isNotEmpty) {
      var item = currencies.firstWhere(
        (element) => element.currencyDisplay == currency,
      );
  ...
  static String getPhoneCodeByPhoneNumber(String phoneNumber) {
    final item = countryPhoneCodes.firstWhereOrNull(
      (e) => phoneNumber.contains(e['dial_code']),
    );
    if (item != null) {
