...
class ImageTools {
  static String prestashopImage(String url, [kSize? size = kSize.medium]) {
    final imageSize = switch (size) {
      kSize.large => 'large_default',
      kSize.small => 'small_default',
      _ => 'medium_default',
    };
    final resizedDirectImage = url.replaceFirstMapped(
      RegExp(
        r'-[^/?#.-]+_default(\.(?:jpe?g|png|webp|gif|bmp|avif))(?=([?#]|$))',
        caseSensitive: false,
      ),
      (match) => '-$imageSize${match.group(1)}',
    );
    if (resizedDirectImage != url) {
      return resizedDirectImage;
    }
    if (RegExp(
    ...
    if (url.contains('?')) {
      return url.replaceFirst('?', '/$imageSize?');
    }
  ...
  static Future<file.File> writeToFile(
    Uint8List? data, {
    String? fileName,
  }) async {
  ...
  static Future<void> preLoadingImage(
    String imageUrl,
