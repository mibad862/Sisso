...
import '../../models/index.dart'
    show BackDropArguments, CartModel, Product, RecentModel, UserModel, User;
...
class NavigateTools {
  ...
  static Future onTapNavigateOptions({
    ...
    if (config.hasTabNumber) {
      ...
      if (tabData != null) {
        ...
        if (!tabData.visible && !tabData.isFullscreen) {
      ...
      return MainTabControlDelegate.getInstance().tabAnimateTo(index);
    }
    if (config.hasScreen) {
      ...
      var appModel = Provider.of<AppModel>(context, listen: false);
      tabData =
          appModel.appConfig?.tabBar.firstWhereOrNull(
            (element) => element.layout == screen,
          ) ??
          tabData;
      final userModel = Provider.of<UserModel>(context, listen: false);
      ...
      return Navigator.of(
        context,
      ).pushNamed(screen, arguments: routeData ?? tabData);
    }
    ...
    if (config.hasUrl) {
      ...
      final categoryMenuShowDepth =
          bool.tryParse('${config['categoryMenuShowDepth']}') ??
          bool.tryParse('${config['categoryShowDepth']}') ??
          false;
      final allowFilterMultipleCategory = bool.tryParse(
        '${config['allowFilterMultipleCategory']}',
      );
  ...
  static Future<void> navigateToLogin(
    ...
    if (kAuthConfig.smsLoginAsDefault) {
    ...
    await _getFluxNavigate(
      ...
      replacement: replacement,
      context: context,
      arguments: {'showGuestButton': showGuestButton},
    );
  }
  ...
  static void navigateRegister(context, {bool replacement = false}) {
    if (ServerConfig().isSupportAuthWebView) {
      ...
      _pushRouteName(
        context,
        RouteList.memberShipUltimatePlans,
        replacement: replacement,
      );
    ...
    } else if (kAuthConfig.enableDigitsMobileLogin &&
        ServerConfig().isWooPluginSupported) {
      _pushRouteName(
        context,
        RouteList.digitsMobileLoginSignUp,
        replacement: replacement,
      );
    } else {
      if (kAuthConfig.smsLoginAsDefault) {
  ...
  static void navigateToProductDetail(
    ...
    try {
      // Fix case open product from sidemenu, but the sidemenu is closed soon so
    ...
    final isFloatingTabBar =
        context.read<AppModel>().appConfig?.settings.tabBarConfig.floating ==
        true;
    unawaited(
      FluxNavigate.pushNamed(
        ...
        arguments: product,
        context: context,
        forceRootNavigator: isFloatingTabBar,
      ),
    );
  }
...
extension _ExtensionNavigatorToolConfig on Map {
  ...
  bool get requiredLogin => bool.tryParse('${this['requiredLogin']}') ?? false;
  String? get title => this['title']?.toString();
  String? get script => this['script']?.toString();
}
