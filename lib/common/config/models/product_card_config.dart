...
enum StoreStyle {
  ...
  factory StoreStyle.fromString(String? type) {
    final normalized = type?.trim();
    if (normalized == null || normalized.isEmpty) {
      return StoreStyle.hide;
    }

    return StoreStyle.values.firstWhere(
      (style) => style.name == normalized,
      orElse: () => StoreStyle.normal,
    );
  }

  factory StoreStyle.parseStoreStyleFromConfig(
    Map json, {
    StoreStyle fallbackStyle = StoreStyle.normal,
  }) {
    if (json.containsKey('storeStyle')) {
      return StoreStyle.fromString(json['storeStyle']?.toString());
    }

    if (!json.containsKey('hideStore')) {
      return fallbackStyle;
    }

    final oldHideStore = bool.tryParse('${json['hideStore']}') ?? false;
    return oldHideStore ? StoreStyle.hide : StoreStyle.normal;
  }

  bool get isNormal => this == StoreStyle.normal;
  bool get isStyle01 => this == StoreStyle.style01;
  bool get isHide => this == StoreStyle.hide;
}
...
class ProductCardConfig {
  ...
  ProductCardConfig.fromJson(dynamic json) {
    ...
    storeStyle = StoreStyle.parseStoreStyleFromConfig(json);
    ...
    showShortDescription = json['showShortDescription'] ?? false;
    ...
    typeShouldOpenInWebview = List<String>.from(
      json['typeShouldOpenInWebview'] ?? <String>[],
    );
  }
