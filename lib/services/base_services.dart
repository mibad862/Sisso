...
abstract class BaseServices {
  ...
  Future<Map<String, dynamic>> processDokanPayPalPayment({
    ...
    throw UnimplementedError(
      'processDokanPayPalPayment is not implemented for this service',
    );
  }
  ...
  Future<CartDataShopify?> addCartLines({
    required String cartId,
    required List<Map<String, dynamic>> lines,
  }) async => null;

  Future<CartDataShopify?> updateCartLines({
    required String cartId,
    required List<Map<String, dynamic>> lines,
  }) async => null;

  Future<CartDataShopify?> removeCartLines({
    required String cartId,
  ...
  Future<List<OrderNote>>? getOrderNote({String? userId, String? orderId}) =>
      null;
  ...
  Future<List<FilterFeature>>? getFilterFeatures({
    String? categoryIds,
    String? tagIds,
    String? brandIds,
  }) => null;

  Future<List<SubFeature>>? getSubFeatures({
    required int id,
    int page = 1,
    int perPage = 10,
  }) => null;
  ...
  Future<String?> generateProductTagUrl(dynamic productTagId) async => null;
  ...
  Future<List<Store>>? searchStores({String? keyword, int? page}) => null;

  Future<List<Store>>? getFeaturedStores({int? page, int? perPage}) => null;
  ...
  Future<void>? cancelBooking({required String bookingId}) => null;

  Future<void>? deleteBooking({required String bookingId}) => null;
  ...
  /// {@macro fluxstore.blog_service.warning_override}
  @nonVirtual
  Future<List<Blog>> searchBlog({required String name, bool? boostEngine}) =>
  ...
  }) async => const PagingResponse<Category>();
  ...
  Future<PagingResponse<Product>> getProductsByCategoryId(
    ...
    String? orderBy,
    String? order,
  }) async => const PagingResponse<Product>();
  ...
  Future<PagingResponse<Review>> getListReviewByUserEmail(
    ...
    int perPage = 20,
    String? status,
  }) async => const PagingResponse(data: []);
  ...
  Future<List<ListingRegion>> getListingRegions() async {
    return <ListingRegion>[];
  }

  Future<ListeoFieldsEditorResponse> getListeoFieldsEditor({
    String? listingId,
    String? type,
  }) async {
    return ListeoFieldsEditorResponse.empty;
  }
  ...
  Future<int> setOffTime(String cookie, bool isAvailable) async => 0;
}
