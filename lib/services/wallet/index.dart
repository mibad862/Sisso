...
class TeraWalletServices extends WalletServices {
  ...
  @override
  Future processPayment({required String orderId, String? token}) async {
    try {
  ...
  @override
  Future partialPayment({required String orderId, String? token}) async {
    try {
      var response = await httpPost(
  ...
  Future<Product?> checkRecharge({required int amount}) async {
    try {
      var response = await httpPost(
        ...
        body: convert.jsonEncode({'amount': amount}),
  ...
  Future<List<WalletRequest>?> getApprovedRequests({
    ...
    try {
      ...
      var response = await httpGet(
        Uri.parse('$domain/wp-json/api/flutter_tera_wallet/approved_requests'),
        refreshCache: true,
        headers: {'User-Cookie': EncodeUtils.encodeCookie(token)},
      );
      var jsonDecode = convert.jsonDecode(response.body);
  ...
  Future<List<WalletRequest>?> getCancelledRequests({
    ...
    try {
      var list = <WalletRequest>[];
      var response = await httpGet(
        Uri.parse('$domain/wp-json/api/flutter_tera_wallet/cancelled_requests'),
        refreshCache: true,
        headers: {'User-Cookie': EncodeUtils.encodeCookie(token)},
      );
      var jsonDecode = convert.jsonDecode(response.body);
  ...
  Future<bool> savePaymentSettings({
    ...
    try {
      var response = await httpPost(
        ...
        body: convert.jsonEncode({'payment_method': paymentMethod, ...fields}),
        headers: {'User-Cookie': EncodeUtils.encodeCookie(token)},
      );
      var jsonDecode = convert.jsonDecode(response.body);
      if (jsonDecode is Map && isNotBlank(jsonDecode['message'])) {
