import 'package:flutter/material.dart';
import 'package:flux_interface/flux_interface.dart';
import 'package:provider/provider.dart';

import '../../models/index.dart';
import '../../modules/analytics/abstract_analytic_trigger.dart';
import '../services.dart';

class FirebaseAnalytic extends AbstractAnalyticTrigger {
  FirebaseAnalyticsService? get _firebaseAnalyticsService =>
      Services().firebase.firebaseAnalytics;

  String? _resolveCurrencyCode(BuildContext? context) {
    if (context == null) {
      return null;
    }

    final appModel = context.read<AppModel>();
    final currencyCode = appModel.currencyCode;
    if (currencyCode?.isNotEmpty ?? false) {
      return currencyCode!.toUpperCase();
    }

    return appModel.currency;
  }

  @override
  void onLogin(String? method) {
    _firebaseAnalyticsService?.logCustomEvent(
      name: 'login',
      parameter: {'method': method ?? 'unknown'},
    );
  }

  @override
  void onRegister(String? method) {
    _firebaseAnalyticsService?.logCustomEvent(
      name: 'sign_up',
      parameter: {'method': method ?? 'unknown'},
    );
  }

  @override
  void onTapProduct(Product value, String? itemListId, String? itemListName) {
    _firebaseAnalyticsService?.logSelectItem(
      itemListId: itemListId,
      itemListName: itemListName,
      data: value,
    );
  }

  @override
  void onAddToCart(Product value, int? quantity, BuildContext? context) {
    _firebaseAnalyticsService?.logAddToCart(
      currency: _resolveCurrencyCode(context),
      price: double.tryParse(value.price ?? ''),
      data: value,
      quantity: quantity,
    );
  }

  @override
  void addToWishList(Product value) {
    _firebaseAnalyticsService?.logAddToWishlist(data: value);
  }

  @override
  void onViewCart(Map value, BuildContext? context) {
    String? currency;
    double? price;
    if (context != null) {
      final model = context.read<CartModel>();
      currency = model.currencyCode;
      price = model.getSubTotal();
    }
    _firebaseAnalyticsService?.logViewCart(
      currency: currency,
      data: value,
      price: price,
    );
  }

  @override
  void onRemoveProductFromCart(
    Product value,
    int? quantity,
    BuildContext? context,
  ) {
    _firebaseAnalyticsService?.logRemoveFromCart(
      currency: _resolveCurrencyCode(context),
      data: value,
      price: double.tryParse(value.price ?? ''),
    );
  }

  @override
  void onViewProduct(Product value, BuildContext? context) {
    _firebaseAnalyticsService?.logViewItem(
      currency: _resolveCurrencyCode(context),
      data: value,
      price: double.tryParse(value.price ?? ''),
    );
  }

  @override
  void onBeginCheckout(BuildContext context) {
    final model = context.read<CartModel>();
    _firebaseAnalyticsService?.logBeginCheckout(
      coupon: model.couponObj?.code,
      currency: model.currencyCode,
      data: model.getProductsInCart(),
      price: model.getSubTotal(),
    );
  }

  @override
  void onPurchased(Order value, BuildContext? context) {
    double? price;
    double? shipping;
    double? tax;
    String? coupon;
    String? currency;
    List? productList;

    if (context != null) {
      final model = context.read<CartModel>();
      price = model.getSubTotal();
      shipping = model.getShippingCost();
      tax = model.taxesTotal;
      coupon = model.couponObj?.code;
      currency = model.currencyCode;
      productList = model.getProductsInCart();
    }

    // Fallback to order payload when checkout flow clears cart early.
    price ??= value.total;
    shipping ??= value.totalShipping;
    tax ??= value.totalTax;
    currency ??= value.currencyCode;
    productList ??= value.lineItems;

    _firebaseAnalyticsService?.logPurchase(
      orderId: value.id,
      price: price,
      shipping: shipping,
      tax: tax,
      coupon: coupon,
      currency: currency,
      data: productList,
    );
  }

  @override
  void onAddPaymentInfo(String? value, BuildContext? context) {
    double? price;
    String? coupon;
    String? currency;
    List? productList;
    if (context != null) {
      final model = context.read<CartModel>();
      price = model.getSubTotal();
      coupon = model.couponObj?.code;
      currency = model.currencyCode;
      productList = model.getProductsInCart();
    }
    _firebaseAnalyticsService?.logAddPaymentInfo(
      coupon: coupon,
      currency: currency,
      data: productList,
      paymentType: value,
      price: price,
    );
  }

  @override
  void onCreateRefund(Order value) {
    _firebaseAnalyticsService?.logRefund(
      orderId: value.id,
      price: value.total,
      currency: value.currencyCode,
      data: value.lineItems,
    );
  }

  @override
  void onAddShippingInfo(String? value, BuildContext? context) {
    double? price;
    String? coupon;
    String? currency;
    List? productList;
    if (context != null) {
      final model = context.read<CartModel>();
      price = model.getSubTotal();
      coupon = model.couponObj?.code;
      currency = model.currencyCode;
      productList = model.getProductsInCart();
    }
    _firebaseAnalyticsService?.logAddShippingInfo(
      coupon: coupon,
      currency: currency,
      data: productList,
      price: price,
      shippingTier: value,
    );
  }

  @override
  void onViewListProduct(List<Product> value, BuildContext? context) {
    String? itemListId;
    String? itemListName;
    if (context != null) {
      final model = context.read<ProductModel>();
      itemListId = model.categoryIds?.join(',') ?? model.tagIds?.join(',');
      itemListName = model.categoryName;
    }

    _firebaseAnalyticsService?.logViewItemList(
      itemListId: itemListId,
      itemListName: itemListName,
      data: value,
    );
  }

  @override
  void onSelectPromotion(String? promotionId, String? promotionName) {
    _firebaseAnalyticsService?.logSelectPromotion(
      promotionId: promotionId,
      promotionName: promotionName,
    );
  }
}
