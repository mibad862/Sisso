...
class _StateSideMenu extends BaseScreen<SideMenu> {
  ...
  void initState() {
    ...
    _subCloseNativeDrawer = eventBus.on<EventCloseNativeDrawer>().listen((
      event,
    ) {
      if (_scaffoldKey.currentState!.isDrawerOpen) {
  ...
  Widget renderLayout({bool enableDrawer = false}) {
    return Scaffold(
                      ...
                      children: [
                        ...
                        if (widget.colorFilter?.isNotEmpty ?? false)
                          Container(
                            width: MediaQuery.of(context).size.width,
                            height: MediaQuery.of(context).size.height,
                            color: HexColor(widget.colorFilter)
                                .withValueOpacity(
      ...
      onDrawerChanged: (isOpen) {
        OverlayControlDelegate().hideSmartChat.value = isOpen;
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    if (widget.zoomConfig != null) {
      ...
      var colorFilter = (widget.colorFilter?.isNotEmpty ?? false)
          ? HexColor(
              widget.colorFilter,
            ).withValueOpacity(widget.filter?.toDouble() ?? 0.0)
          : null;
      return Stack(
        children: [
          if (color != null)
          ...
          Material(
            color: Theme.of(context).colorScheme.surface.withValueOpacity(
              enableBackground ? 0.0 : 1.0,
            ),
            child: ZoomDrawer(
              ...
              mainScreenScale: mainScreenScale(
                zoomConfig.style,
                zoomConfig.mainScreenScale.toDouble(),
              ),
