...
class MainTabsState extends CustomOverlayState<MainTabs>
    ...
    if (appSetting.ageRestrictionConfig.enable &&
      ...
      Future.delayed(const Duration(seconds: 3), () {
        FluxNavigate.push(
          MaterialPageRoute(
            builder: (context) =>
                AgeRestrictionScreen(config: appSetting.ageRestrictionConfig),
  ...
  void _initListenEvent(context) {
    ...
    _subCloseCustomDrawer = eventBus.on<EventCloseCustomDrawer>().listen((
      event,
    ) {
      setState(() {
    ...
    _subLoadedAppConfig = eventBus.on<EventLoadedAppConfig>().listen((
      event,
    ) async {
      try {
  ...
  Future<bool> _handleWillPopScopeRoot() async {
    if (tabController.index == currentTabIndex) {
      final tab = tabData[currentTabIndex];
      if (tab.layout == RouteList.page) {
        final isBack = await AppBarWebControlDelegate.onBack?.call();
        if (isBack == true) {
          return false;
        }
      }
  ...
  Future<bool> _exitApp() async {
    final exitAppType = kAdvanceConfig.exitAppType;
    if (exitAppType.isSystemDefault) {
      await NavigateTools.exitApp();
      return false;
    } else if (exitAppType.isExitImmediately) {
      return NavigateTools.forceExitApp();
    } else if (exitAppType.isRequireDoubleTap) {
      final now = DateTime.now();
      const snackBarDisplayDuration = Duration(milliseconds: 2000);
      if (currentBackPressTime == null ||
          (now.difference(currentBackPressTime!) > snackBarDisplayDuration)) {
        currentBackPressTime = now;
        Tools.showSnackBar(
          ScaffoldMessenger.of(context),
          S.of(context).pressBackButtonAgainToExit,
  ...
  void dispose() {
    ...
    Services().disposeConnectivityService();
  ...
  Widget build(BuildContext context) {
    ...
    final showMinimizeTabBar = tabBarConfig.enable && tabBarConfig.showMinimize;
    final showFloating = tabBarConfig.showFloating;
    final floating = tabBarConfig.floating;
    ...
    return Consumer<BottomBarModel>(
      builder: (_, bottomBarModel, child) {
        return SideMenu(
          extendBody: bottomBarModel.isActive || floating,
          backgroundColor: (showFloating || floating)
              ? null
          ...
          drawer: (appConfigDrawer?.enable ?? true)
              ? const SideBarMenu()
              : null,
                          ...
                          child: Stack(
                            ...
                            children: [
                              if (lazyLoading)
                                ...
                                ...List.generate(_tabView.length, (index) {
...
extension TabBarMenuExtention on MainTabsState {
  Object? _getRouteDataByTab(TabBarMenuConfig? tabData) {
    final appModel = context.read<AppModel>();
    final tabBar = appModel.appConfig?.tabBar;
    final groupTabData = tabBar?.where((e) => e.groupLayout == true).toList();
    var routeData =
        tabData?.layout == RouteList.tabMenu ||
  ...
  void _onChangeTab(String? nameTab, {bool allowPush = true}) {
    if (saveIndexTab[nameTab] != null) {
      ...
      final tabBar = appModel.appConfig?.tabBar;
      final tabData = tabBar?.firstWhereOrNull((e) => e.layout == nameTab);
      final routeData = _getRouteDataByTab(tabData);
  ...
  /// init Tab Delegate to use for SmartChat/Ads/secure screen features
    ...
    return Navigator(
      ...
      observers: [
        MyRouteObserver(
          action: (screenName) {
            ...
            /// Only report to overlay consumers (ads, smart chat, secure
            /// screen) when this tab is the visible one. During a tab rebuild
            /// (e.g. after a language change) every tab's navigator
            /// regenerates its initial route and would otherwise emit a
            /// non-visible screen, overriding the real current screen.
            if (saveIndexTab[initialRoute] != tabController.index) return;
      ...
      onGenerateRoute: (RouteSettings settings) {
        if (settings.name == initialRoute) {
          return Routes.getRouteGenerate(
  ...
  void _initTabData(context) async {
    ...
    var tabView = <Widget>[];
    saveIndexTab = {};
    // Navigators are recreated below (new keys -> reset to their initial
    // route), so any previously tracked in-tab screen is stale. Clear it so
    // the active tab re-emits its real current screen after the rebuild.
    childTabName.clear();
    hasDefaultTab = false;
    ...
    for (var i = 0; i < tabData.length; i++) {
      ...
      var routeData =
          initialRoute == RouteList.tabMenu ||
