...
class _AppBarItemWidgetState extends State<AppBarItemWidget> {
  ...
  // This Address is used to display current selected address in the location
  // item of the app bar only. Do not use it to update the CartModel address.
  ...
  Future<void> _onTapItem(AppBarItemConfig item) async {
    ...
    } else if (item.type == 'multisites_switcher') {
      await MultiSiteFactory.instance.startShowMultiSiteSelection(context);
    } else {
  ...
  void afterFirstLayout(BuildContext context) {
    ...
    Future.microtask(() async {
      ...
      if (newAddress.country != null) {
        final countryName = await Services().widget.getCountryName(
          context,
          newAddress.country,
        );
        ...
        if (countryName != null) {
          ...
          if (newAddress.state != null) {
            ...
            if (state != null) {
              newAddress = newAddress.copyWith(state: state.name);
            }
          }
        }
      }
    ...
    switch (item.type) {
      case 'avatar':
        widget = Padding(
          ...
          child: AvatarWidget(
            ...
            imageColor: item.imageColor != null
      ...
      case 'location':
        widget = Container(
          ...
          child: Selector<CartModel, Address?>(
            selector: (_, cartModel) => cartModel.address,
        ...
        widget = Container(
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              SizedBox(width: 4.0, height: appBarSize),
              if (!item.hideTitle)
                Expanded(
                  child: Text(
                    item.title ?? '',
      ...
      case 'icon':
        widget = SizedBox(
          ...
          child: IconButton(
            ...
            icon: DeferredWidget(
              defer_icon.loadLibrary,
              () => Icon(defer_icon.iconPicker(item.icon!, item.fontFamily!)),
            ),
