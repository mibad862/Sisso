...
mixin ProductVariantMixin on BaseFrameworks {
  ProductVariation? updateVariation(
    ...
    final templateVariation = variations.isNotEmpty
        ? variations.firstWhereOrNull((element) {
    ...
    var validAttribute = listAttributes.lastWhereOrNull((attributes) {
    ...
    validAttribute ??= listAttributes.lastWhereOrNull((attributes) {
      final valueCheck = attributes.map((e) => e.toStringCompare()).join();
    ...
    productVariation = variations.lastWhere(
      (element) => element.attributes == validAttribute,
    );

    for (var element in productVariation.attributes) {
      if (!mapAttribute.containsKey(element.keyAttr)) {
        mapAttribute[element.keyAttr] = element.option!;
      }
    }
  ...
  List<Widget> makeProductTitleWidget(
    ...
    if (isAvailable) {
      listWidget.add(const SizedBox(height: 5.0));
            ...
            if ((kProductDetail.showSku) && (sku?.isNotEmpty ?? false)) ...[
              ...
              Expanded(
                child: GestureDetector(
                  onTap: () {
                    Clipboard.setData(ClipboardData(text: sku!)).then((value) {
      ...
      final allowBackorder = productVariation != null
          ? (productVariation.backordersAllowed ?? false)
          : product.backordersAllowed;
      // Backorder status only applies when backorders are allowed AND the
      // product is actually out of stock. If stock is still available (inStock
      // is true), we show In Stock even if backorders are enabled.
            ...
            if (kProductDetail.showStockStatus) ...[
              ...
              Text(
                inStock
                    ...
                    : isOnBackorder
                    ? '${S.of(context).backOrder}$stockQuantity'
                    : S.of(context).outOfStock,
                style: Theme.of(context).textTheme.titleSmall!.copyWith(
                  color: inStock
                      ? kStockColor.inStock
                      : isOnBackorder
                      ? kStockColor.backorder
      ...
      if (productVariation?.description?.isNotEmpty ?? false) {
        listWidget.add(
          Services().widget.renderProductDescription(
            context,
      ...
      if (product.shortDescription != null &&
        ...
        listWidget.add(ProductShortDescription(product));
      }

      listWidget.add(const SizedBox(height: 15.0));
    }
  ...
  List<Widget> makeBuyButtonWidget({
    ...
    if (isVariationLoading) {
      ...
      final buttonText = product.buttonText?.trim() ?? '';
      text = buttonText.isNotEmpty ? buttonText : S.of(context).buyNow;
    } else if (alwaysShowBuyButton || (allowToBuy && isAvailable)) {
    ...
    final downloadItems = product.downloadItems ?? [];
    if ((product.isPurchased) &&
        (product.isDownloadable ?? false) &&
        downloadItems.isNotEmpty &&
        (downloadItems.first.file?.isNotEmpty ?? false)) {
      return [
        Row(
          children: [
            Expanded(
              child: InkWell(
                onTap: () async => await Tools.launchURL(
                  downloadItems.first.file,
  ...
  void addToCart(
    ...
    var productVariation = Provider.of<ProductModel>(
      context,
      listen: false,
    ).selectedVariation;
    (success, message) = await cartModel.addProductToCart(
      ...
      cartItemMetaData: CartItemMetaData(
        ...
        selectedTieredPrice: args?.selectedTieredPrice,
        tieredPrices: args?.tieredPrices,
        pwGiftCardInfo: args?.pwGiftCardInfo,
        selectedYithOptions: args?.selectedYithOptions,
...
Widget actionButton(
  ...
  return DisableWidget(
    showColorFilter: !isAvailable || disableBuyButton,
    disable: isInAppPurchaseChecking || disableBuyButton,
    child: Row(
      children: <Widget>[
        Expanded(
          child: GestureDetector(
