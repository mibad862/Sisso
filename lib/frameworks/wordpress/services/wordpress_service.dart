...
import '../../../services/wordpress/wordpress_connector.dart';

class WordPressService extends BaseServices {
  late final WordpressConnector _connector;

  WordPressService({required super.domain, bool? isRoot})
    : isSecure = domain.contains('https') ? '' : '&insecure=cool',
      _connector = WordpressConnector(
        WordpressApi(domain, isRoot: isRoot ?? true),
      ),
      super(
  ...
  Future<User> loginSMS({String? token}) async {
    try {
      ...
      var response = await _connector.httpPost(
        endPoint.toUri()!,
  ...
  Future<User?> loginGoogle({String? token}) async {
    ...
    try {
      ...
      var response = await _connector.httpGet(endPoint);
  ...
  Future<List<Blog>?> getBlogsByUserId(String userId) async {
    try {
      var response = await _connector.getAsync('posts?_embed&author=$userId');
      var list = <Blog>[];
  ...
  Future<Map<String, dynamic>?> getHomeCache(String? lang) async {
    try {
      final jsonData = await _connector.httpGet(
  ...
  Future<bool> deleteAccount(String token) async {
    try {
      final response = await _connector.httpDelete(
