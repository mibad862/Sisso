...
class DokanService extends WooCommerceService {
  ...
  Future<User?> createUser({
    ...
    try {
      ...
      final response = await dokanConnector.httpPost(
        endPoint,
  ...
  @override
  Future<List<Store>>? getFeaturedStores({int? page, int? perPage}) async {
    var list = <Store>[];
    try {
      var response = await dokanConnector.getAsync(
        'stores?page=${page ?? 1}&per_page=${perPage ?? 100}',
      );
  ...
  Future<List<Store>> searchStores({String? keyword, int? page}) async {
    try {
      ...
      final endPoint = StringBuffer(
        'stores?page=${page ?? 1}&per_page=20&status=approved',
      )..write(hasKeyword ? '&search=$trimmedKeyword' : '');

      final response = await dokanConnector.getAsync(endPoint.toString());
