...
class QueryString {
  static Map<String, String> parse(String query) {
    ...
    for (final part in query.split('&')) {
      ...
      final key = index >= 0 ? part.substring(0, index) : part;
      final value = index >= 0 ? part.substring(index + 1) : '';

      result[_safeDecode(key)] = _safeDecode(value);
    }
    return result;
  }
...
class WooCommerceAPI {
  ...
  WooCommerceAPI(this.url, this.consumerKey, this.consumerSecret) {
    ...
    if (url!.startsWith('https') &&
        (!kAdvanceConfig.optimizeImage.enable ||
            kAdvanceConfig.optimizeImage.plugin.isRegenerateImagePlugin)) {
      //if use Optimole plugin, must use oauth instead of adding consumer_key & consumer_secret to url, it will not return image url with cdn url
      isHttps = true;
    } else {
      isHttps = false;
    }
