...
class _UserUpdateScreenState extends State<UserUpdateWooScreen> {
  ...
  Widget renderBackgroundAvatar(dynamic value) {
    ...
    if (value is String && value.isNotEmpty) {
      avatar = FluxImage(imageUrl: value, fit: BoxFit.cover);
    }
    if (value is XFile) {
  ...
  String? validatorEmail(String? value, String label) {
  ...
  Widget buildLoading(UserUpdateModel model) {
    return model.state == UserUpdateState.loading
        ? Container(
            ...
            child: const Center(
              child: SpinKitCircle(color: Colors.white, size: 20.0),
            ),
          )
        : const SizedBox();
  }
