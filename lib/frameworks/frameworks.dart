...
import '../models/entities/b2bking/tiered_price.dart';
...
abstract class BaseFrameworks {
  ...
  bool get isRequiredLogin => kAuthConfig.isRequired;
  ...
  /// Use this method to load app config from Mstore API caching after loading
  /// the local app config
  Future<void>? onLoadedAppConfig(String? lang, Function(Map?) callback) =>
      null;
  ...
  /// This method should verify the current password before updating to new password
  ...
  Future<bool> changePassword({
    ...
    // Default implementation returns false - should be overridden by specific frameworks
    onError?.call('Password change not supported in this framework');
    return false;
  }

  Widget renderShippingPaymentTitle(
    BuildContext context,
    String title, {
  ...
  Widget renderTaxes(TaxModel taxModel, BuildContext context) {
    final currencyRate = Provider.of<AppModel>(
      context,
      listen: false,
    ).currencyRate;
    final currency = Provider.of<CartModel>(
      context,
      listen: false,
    ).currencyCode;
      ...
      list.add(
        Row(
          ...
          children: <Widget>[
            HtmlWidget(
              element.name ?? '',
              textStyle: TextStyle(fontSize: 14, color: secondaryColor),
            ),
            Text(
              PriceTools.getCurrencyFormatted(
                element.total,
  ...
  Widget renderCategoryLayout({
    ...
    switch (layout) {
      ...
      case ColumnCategories.type:
        return ColumnCategories(scrollController: scrollController);
      case SubCategories.type:
        return SubCategories(scrollController: scrollController);
      ...
      case GridCategory.type:
        return GridCategory(scrollController: scrollController);
      ...
      default:
        return HorizonMenu(scrollController: scrollController);
    }
  }
  ...
  Widget renderShippingMethodInfo(
    BuildContext context, {
                ...
                Text(
                  PriceTools.getCurrencyFormatted(
                    cartModel.getShippingCost(
                      includeTax:
                          cartModel.shippingMethod?.isIncludingTax ?? true,
                    ),
                    currencyRate,
  ...
  Widget renderRewardInfo(BuildContext context) {
    ...
    final rewardTotal = Provider.of<CartModel>(
      context,
      listen: false,
    ).rewardTotal;
  ...
  Widget renderSearchScreen({bool? boostEngine, bool? showQRCode}) {
    return SearchScreen(
  ...
  Widget renderVendorAdmin(BuildContext context, User user) {
    ...
    final webView = WebView(
      ...
      script: kWebViewConfig.webViewScript,
    );
  ...
  Widget renderProductCardView({
    ...
    switch (cardDesign) {
      ...
      default:
        if (cardDesign.isSimpleForWeb && useDesktopStyle) {
  ...
  Widget renderProductItemTileView({
    ...
    return ProductItemTileView(item: item, padding: padding, config: config);
  }
  ...
  Widget renderProductGlassView({
    ...
    return ProductGlass(item: item, width: width, config: config);
  }
  ...
  Widget renderDelivery(BuildContext context, User user) {
    ...
    final deliveryDashboardUrl = kVendorConfig.deliveryDashboardUrl.isNotEmpty
        ...
        : '$serverConfigUrl/${['listeo', 'dokan', 'woo'].contains(typeName) ? 'my-account/driver-dashboard/' : 'my-account?delivery=true'}';
  ...
  /// Support External/Affiliate product
  Future<void> openExternal(BuildContext context, Product product) async {
    final url = Tools.prepareURL(product.externalUrl);
      ...
      try {
        if (kIsWeb || Tools.needToOpenExternalApp(url)) {
          ...
          await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => WebView(url, title: product.name),
            ),
          );
        }
        return;
      ...
      MaterialPageRoute(
        builder: (context) {
          return Scaffold(
            ...
            body: Center(child: Text(S.of(context).notFound)),
          );
        },
      ),
    );
  ...
  Widget renderHorizontalListItem(
    Map<String, dynamic> config, {
    cleanCache = false,
  }) {
    if (config['type'] == 'blog') {
      return BlogListLayout(config: BlogConfig.fromJson(config));
    }
    return ProductList(
      config: ProductConfig.fromJson(config),
  ...
  Widget renderHomeSearchScreen() => const SearchScreen();
  ...
  Widget renderCommentLayout(dynamic postId, kBlogLayout type) {
    ...
    return CommentLayout(postId: postId, type: type);
  }
  ...
  Widget renderDetailPrice(
    ...
    if (product.rentalDateSelectionRequired &&
        kAdvanceConfig.enableRentalProductsWoo) {
      return Services().renderRentalDetailPrice(
        product: product,
        axisType: axisType,
      );
    }
    ...
    if (product.yithAddOns?.isNotEmpty ?? false) {
      ...
      if (model.selectedYithOptions.isNotEmpty) {
        return YithAddonsPrice(product: product, priceData: priceData);
      }
    }
  ...
  Widget renderProductDescription(BuildContext context, String description) {
    return HtmlWidget(
      description.unescape(),
      textStyle: Theme.of(context).textTheme.bodyMedium!
          .copyWith(color: Theme.of(context).colorScheme.secondary, height: 1.5)
  ...
  List<OrderByType> getSupportedSortByOptions({bool returnDefault = false}) {
    ...
    // Return default options if no config override
    return [
      OrderByType.onSale,
  ...
  Widget renderFilterSortBy(
    ...
    final supportedSortByOptions = getSupportedSortByOptions();
    final children = [
      if ([
        OrderByType.onSale,
        OrderByType.featured,
        ...
        GroupCheckBoxWidget<String>(
          ...
          values: <GroupCheckBoxItem<String>>{
            if (supportedSortByOptions.contains(OrderByType.onSale))
              GroupCheckBoxItem(title: S.of(context).onSale, value: 'onSale'),
      ...
      GroupCheckBoxWidget<String>(
        ...
        values: <GroupCheckBoxItem<String>>{
          ...
          if (supportedSortByOptions.contains(OrderByType.rand))
            GroupCheckBoxItem(title: S.of(context).random, value: 'rand-null'),
          if (supportedSortByOptions.contains(OrderByType.relevance)) ...[
            GroupCheckBoxItem(
              title: S.of(context).relevance,
  ...
  Widget renderCODExtraFee(BuildContext context) {
    final cartModel = Provider.of<CartModel>(context, listen: false);
    final enabled =
        (kPaymentConfig.smartCOD?.enabled ?? false) &&
    ...
    return ExpansionInfo(
      ...
      children: <Widget>[
        Container(
          ...
          child: Wrap(
            children: List.generate(product!.distinctCategories.length, (
              index,
            ) {
  ...
  Widget renderLoyaltyCouponInfo(BuildContext context) {
    ...
    final currency = Provider.of<AppModel>(context, listen: false).currency;
    final loyaltyCoupon = Provider.of<CartModel>(
      context,
      listen: false,
    ).loyaltyCoupon;
  ...
  String calculateOrderSubtotal({
    ...
    return PriceTools.getCurrencyFormatted(
      order.lineItems.fold(0, (dynamic sum, e) => sum + double.parse(e.total!)),
      currencyRate,
      currency: currencyCode,
    )!;
  }

  Widget renderAccountScreen() => const SizedBox();

  Widget renderChangePasswordScreen() => const SizedBox();

  Widget renderPersonalInfoScreen() => const SizedBox();

  Widget renderResetPasswordScreen(String? resetUrl) => const SizedBox();
}
...
class AddToCartArgs {
  ...
  final List<B2BKingTieredPrice>? tieredPrices;
