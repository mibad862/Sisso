...
class _HeaderActionWebState extends State<HeaderActionWeb> with WebLayoutMixin {
  ...
  void _onTapSupport() => onTapOpenUrl(
    context,
  ...
  void _onTapDownloadPage() => onTapOpenUrl(
    context,
    _appModel.appConfig?.settings.downloadApp ?? kAdvanceConfig.downloadPageUrl,
  );
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      color: _backgroundColor
          .withBlue(
            (_backgroundColor.b + (_backgroundColor.b > sub ? -sub : 0))
                .toInt(),
          )
          .withGreen(
            (_backgroundColor.g + (_backgroundColor.g > sub ? -sub : 0))
                .toInt(),
          )
          .withRed(
            (_backgroundColor.r + (_backgroundColor.r > sub ? -sub : 0))
                .toInt(),
          ),
    ...
    return [
      if (loggedIn)
        _HeaderButtonAction(
          ...
          iconWidget: CircleAvatar(
            backgroundImage: NetworkImage(
              (user?.picture ?? kDefaultImage).addWebProxy(),
            ),
            radius: 12,
          ),
        )
        ...
        _HeaderButtonAction(
          S.of(context).signIn,
          onTap: _onTapSignIn,
          iconWidget: const CircleAvatar(radius: 12, child: Icon(Icons.person)),
