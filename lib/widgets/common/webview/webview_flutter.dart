...
class StateWebviewFlutter extends StateWebviewInterface<WebviewFlutter>
  ...
  void _handleCameraPermissionListener() async {
    if (isAndroid) {
      final androidController =
          _controller.platform as AndroidWebViewController;

      await androidController.setOnPlatformPermissionRequest(
        (request) => _permissionRequest(request),
      );
    }
  }
  ...
  Future<void> _permissionRequest(
    ...
    var hasCamera = types.contains(WebViewPermissionResourceType.camera);
    var hasMic = types.contains(WebViewPermissionResourceType.microphone);

    // Check camera and microphone permissions if requested
    if (hasCamera) {
    ...
    // Other permission types (e.g., protected media, MIDI) are granted by default
    // Geolocation is handled separately in _handleGeolocationPermissionListener
    await request.grant();
  }
  ...
  //       final attachments = await FilePicker.pickFiles();
  ...
  //           .toList();
  //     } else {
  //       final attachment = await FilePicker.pickFile();
  ...
  void initState() {
    ...
    if (url?.isNotEmpty ?? false) {
      _controller
          ...
          NavigationDelegate(
            ...
            onNavigationRequest: (NavigationRequest request) async {
              ...
              if (url.startsWith('data:image')) {
                await saveBase64ToImage(request.url);
        ...
        ..loadRequest(Uri.parse(url.toString()), headers: widget.headers ?? {});
    ...
    _handleCameraPermissionListener();
