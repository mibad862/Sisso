...
import 'mixin/secure_screen_mixin.dart';
...
class OverlayControlDelegate {
  ...
  /// Force-hide the floating smart chat regardless of the current screen.
  /// Useful to prevent it from overlapping other overlays (e.g. an engagement
  /// popup banner). Set to `true` to hide, `false` to show again.
  final ValueNotifier<bool> hideSmartChat = ValueNotifier(false);
...
abstract class CustomOverlayState<T extends StatefulWidget>
    ...
    with SmartChatMixin, SecureScreenMixin, TickerProviderStateMixin {
  ...
  TabBarConfig? get tabBarConfig => Provider.of<AppModel>(
    context,
    listen: false,
  ).appConfig?.settings.tabBarConfig;
    ...
    WidgetsBinding.instance.endOfFrame.then((_) {
      if (mounted) {
        EasyDebounce.debounce(_tag, const Duration(seconds: 2), () {
          ...
          OverlayControlDelegate().emitTab = (tabName) =>
              _handleBottomBarHeight(tabName, onBottomBar: true);
  ...
  void _handleBottomBarHeight(String? screenName, {bool onBottomBar = false}) {
    ...
    if (isExcludeRoute || onBottomBar) {
      ...
      if (screenName == RouteList.dashboard) {
        final currentTabName = MainTabControlDelegate.getInstance()
            .currentTabName();
    ...
    eventBus.fire(EventScreenChanged(screenName: routePath));
    Services().advertisement.handleAd(routePath ?? '');
    handleSmartChat(routePath?.isNotEmpty == true ? routePath : null);
    handleSecureScreen(routePath?.isNotEmpty == true ? routePath : null);
    overlayController.sink.add(true);
  }

  OverlayEntry _buildOverlayEntry() {
    final overlayMarginBottom =
        (tabBarConfig?.paddingTop ?? 0) +
                ...
                child: Column(
                  ...
                  children: [
                    ValueListenableBuilder<bool>(
                      valueListenable: OverlayControlDelegate().hideSmartChat,
                      builder: (_, hideChat, child) {
                        // Force-hide the smart chat (e.g. while an engagement
                        // popup banner is displayed) so it does not overlap.
                        final showMinimize =
                            tabBarConfig?.showMinimize ?? false;
                        if (hideChat || showMinimize) {
                          return const SizedBox();
                        }
                        return child!;
                      },
                      child: buildSmartChatWidget(),
                    ),
