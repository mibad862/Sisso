...
class MultiAttributeFilterWidgetState
  ...
  @override
  void initState() {
    _listSelectedAttributes = Map<FilterAttribute, List<SubAttribute>>.from(
  ...
  void _onTapSubAttribute(
    ...
    var selectedSubAttribute = List<SubAttribute>.from(
      _listSelectedAttributes.entries
    ...
    final isExist =
        selectedSubAttribute.indexWhere(
    ...
    setState(() {
      if (isExist) {
        selectedSubAttribute.removeWhere(
          (element) => element.id == subAttribute.id,
        );
      } else {
      ...
      if (selectedSubAttribute.isEmpty) {
        _listSelectedAttributes.removeWhere(
          (key, value) => key.id == attribute.id,
        );
      } else {
  ...
  Widget _renderSubAttributeList(
    ...
    final selectedSubAttributes =
        _listSelectedAttributes.entries
            .firstWhereOrNull((entry) => entry.key.id == attribute.id)
            ?.value ??
        <SubAttribute>[];

    final listVisibleSubAttribute = listSubAttribute
        .where((element) => element.isVisible)
        .toList();

    if (listVisibleSubAttribute.isEmpty) {
      return const SizedBox();
    }

    return Container(
      margin: const EdgeInsets.symmetric(horizontal: 10),
      height: listVisibleSubAttribute.length > 4 ? 100 : 50,
      child: GridView.count(
        scrollDirection: Axis.horizontal,
        childAspectRatio: 0.4,
        shrinkWrap: true,
        crossAxisCount: listVisibleSubAttribute.length > 4 ? 2 : 1,
        children: [
          for (var subAttribute in listVisibleSubAttribute)
            FilterOptionItem(
              title: subAttribute.name ?? '',
              selected: selectedSubAttributes.any(
                (element) => element.id == subAttribute.id,
              ),
