...
class _CategoryTreeState extends State<CategoryFilterWidget> {
  ...
  @override
  void initState() {
    _categoryId = List<String>.from(widget.categoryId ?? categoryId ?? []);
    super.initState();
  }
  ...
  Widget getTreeView({required List<Category> categories}) {
    return TreeView(
      parentList: _getCategoryItems(categories, onFilter: widget.onFilter),
    );
  }
    ...
    final menuCategory = widget.isUseBlog
        ? Selector<BlogModel, List<Category>>(
            builder: (context, categories, child) =>
                getTreeView(categories: categories),
