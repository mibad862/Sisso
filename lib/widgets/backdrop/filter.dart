...
import 'filters/feature_filter_widget.dart';
import 'filters/filters.dart';
import 'filters/multi_feature_filter_widget.dart';
...
class FilterWidget extends StatefulWidget {
  ...
  final Map<FilterFeature, List<SubFeature>>? selectedFeatures;
...
class _FilterWidgetState extends State<FilterWidget> {
  ...
  void _onFilter({
    ...
    listingLocationId: (listingLocationId?.isEmpty ?? false)
        ? null
        : listingLocationId ?? context.read<ProductModel>().listingLocationId,
  ...
  bool get _supportAttribute =>
      !_svConfig.isHaravan && !_svConfig.isListingType && !_svConfig.isShopify;
  bool get _supportFeature => ServerConfig().isPrestashop;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: <Widget>[
                ...
                children: <Widget>[
                  ...
                  Text(
                    ServerConfig().isWordPress
                        ? context.select(
                                (BlogModel blogModel) => blogModel.categoryName,
                              ) ??
                              S.of(context).blog
          ...
          if (widget.showLayout) LayoutFilterWidget(isBlog: isBlog),
          ...
          if (ServerConfig().isListingType && widget.showListingLocation)
          ...
          if (_supportPriceSlider && widget.showPriceSlider)
            PriceSliderFilterWidget(
              ...
              onChanged: (minPrice, maxPrice) {
                _onFilter(minPrice: minPrice, maxPrice: maxPrice);
              },
            ),
          ...
          // Filter by features. Only available for Prestashop
          if (_supportFeature && widget.showFeature)
            if (widget.allowMultiFeature)
              MultiFeatureFilterWidget(
                selectedFeature: widget.selectedFeatures,
                onChanged: (features) {
                  _onFilter(features: features);
                },
              )
            else
              FeatureFilterWidget(
