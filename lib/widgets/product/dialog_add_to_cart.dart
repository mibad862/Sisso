      ...
      showDialog(
        ...
        builder: (context) => Center(
          child: SizedBox(
            ...
            child: _AddToCartBody(product: product, quantity: quantity),
          ),
        ),
      );
      return;
...
class _AddToCartBody extends StatelessWidget {
  const _AddToCartBody({required this.product, this.quantity = 1});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: AutoHideKeyboard(
        child: Stack(
          children: [
            _RubberAddToCart(product: product, quantity: quantity),
            Align(
              ...
              child: InkWell(
                ...
                child: CircleAvatar(
                  backgroundColor: Theme.of(
                    context,
                  ).colorScheme.surface.withValueOpacity(0.9),
                  child: Icon(
...
class _RubberAddToCart extends StatefulWidget {
  final Product product;
  final int quantity;
  const _RubberAddToCart({required this.product, this.quantity = 1});

  @override
  State<_RubberAddToCart> createState() => _StateRubberAddToCart();
}

class _StateRubberAddToCart extends State<_RubberAddToCart> {
  bool isLoading = true;
            ...
            return CustomScrollView(
              slivers: [
                ...
                SliverToBoxAdapter(
                  child: Column(
                    children: [
                      Services().renderTieredPriceTable(product),
