...
class VideosService {
  ...
  VideosService({this.order, this.orderby});

  Future<List<Product>> refreshItems() async {
    page = 1;
    return _getItems(page);
  }

  Future<List<Product>> loadMoreItems() async {
    page += 1;
    final result = await _getItems(page);
  ...
  Future<List<Product>> _getItems(int page) async {
    try {
      var results = <Product>[];
    ...
    try {
      for (var product in products) {
        ...
        if (videoId.isNotEmpty) {
          ...
          videoUrl =
              videoConfig?.request?.files?.progressive
                  ?.firstWhereOrNull(
