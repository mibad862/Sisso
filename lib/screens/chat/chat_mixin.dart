...
mixin ChatMixin<T extends StatefulWidget> on State<T> {
  ...
  List<Map> get supportedSmartChatOptions {
    ...
    for (final option in kListSmartChat) {
      ...
      final isChatProviderExist = _chatServices.checkProviderExist(
        provider: chatProvider,
      );

      if (isChatProviderExist && _supportChatProvider) {
        final isChatProviderEnable = _chatServices.checkProviderEnabled(
          provider: chatProvider,
        );
  ...
  Future<void> onTapItem(
    ...
    if (isChatProviderExist) {
      await _chatServices.showChatScreen(context, chatProvider);
      return;
    }
    ...
    switch (app) {
      ...
      case 'store':
        if (storeArguments != null) {
          await Navigator.of(
            context,
          ).pushNamed(RouteList.vendorChat, arguments: storeArguments);
        }
        break;
      ...
      case 'wcfmLiveChatStore':
        if (storeArguments != null) {
          await Navigator.of(
            context,
          ).pushNamed(RouteList.wcfmLiveChatStore, arguments: storeArguments);
        }
        break;

      case 'wcfmLiveChat':
        await Navigator.of(context).pushNamed(RouteList.wcfmLiveChat);
        break;
      ...
      default:
        ...
        final snackBar = SnackBar(
          content: Text(S.of(context).canNotLaunch),
  ...
  void _openChatWebview({String? url, required BuildContext context}) {
    Navigator.push(
      context,
  ...
  Future showActionSheet({required BuildContext context}) {
    return showCupertinoModalPopup(
      ...
      builder: (popupContext) => CupertinoActionSheet(
        actions: List.generate(supportedSmartChatOptions.length, (index) {
  ...
  /// Build chat item
  Widget buildItemIcon(Map option, double iconSize, Color iconColor) {
    final iconData = option['iconData'];
    final imageData = option['imageData'] ?? '';
    final storeArguments = option['storeArguments'];

    var icon = imageData.isNotEmpty
        ? FluxImage(imageUrl: imageData, width: iconSize, fit: BoxFit.contain)
