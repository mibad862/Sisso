...
class NotificationRequestScreen extends StatefulWidget {
  const NotificationRequestScreen({super.key, this.onFinish});
...
class _NotificationRequestScreenState extends State<NotificationRequestScreen>
  ...
  Future<void> onAcceptPermission() async {
    await _notificationModel.enableNotification(waitForService: false);
    _gotoNextScreen();
  }

  @override
  Future<void> onDeclinePermission() async {
    await _notificationModel.disableNotification(waitForService: false);
    _gotoNextScreen();
  }

  void _gotoNextScreen() {
    SettingsBox().hasShowPermissionNotification = true;
    ...
    if (kIsShowRequestAgreedPrivacy) {
      Navigator.of(context).pushReplacementNamed(
        ...
        arguments: {'onFinish': _finishStep},
      );
  ...
  void _finishStep([BuildContext? ct]) async {
    final ctCurrent = ct ?? context;
    ...
    await Navigator.of(ctCurrent).pushReplacementNamed(RouteList.dashboard);
  }

  Widget renderIcon({double size = 48}) {
    var icon = config.icon;
    if (icon != null && icon.isNotEmpty) {
      return FluxImage(imageUrl: icon, height: size, fit: BoxFit.scaleDown);
    }
    return Icon(permissionIcon, size: size, color: Colors.amber);
  }
