...
class StateProductCategoryMenu extends State<ProductCategoryMenu> {
              ...
              builder: (context, isLoading, child) {
                ...
                if (showDepth) {
                  ...
                  if (firstParent != null) {
                    ...
                    if (level != 0) {
                      final bodyLevel1 = _renderParentForLevel1(
                        firstParent,
                        firstCategory,
                      );
                      final subCategory = _getSubCategories(categoryId);
                      final categorySelected = _selectedCategory?.id;
                      final parentCategorySelected = _selectedCategory?.parent;
  ...
  Widget? _renderParentForLevel1(String parentCategoryId, String categoryId) {
    ...
    final categories = List<Category>.from([?category, ...subCtg]);
          ...
          child: DefaultTabController(
            ...
            child: TabBar(
              ...
              tabs: List.generate(categories.length, (int index) {
                ...
                return Padding(
                  padding: EdgeInsets.symmetric(horizontal: showImage ? 8 : 16),
                  child: showImage
                      ? Column(
                          ...
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                  width: 1.5,
                                  color: category.id == _selectedCategory?.id
                                      ? Theme.of(context).primaryColor
                                      : Colors.transparent,
                                ),
                              ),
  ...
  Widget? _renderWhenParentOfSelectedCategoryIdIsNull(
    ...
    if (_listSubCategory.isNotEmpty) {
      // Create a "See All" option as the first item
      _selectedCategory = _selectedCategory?.copyWith(
        name: S.of(context).seeAll,
      );
  ...
  Widget _renderWhenSelectedCategoryNotYetSubCategory(
    ...
    final parentCategory = getCategoryById(
      parentOfSelectedCategoryId.toString(),
    ...
    return _renderSiblingCategoriesFromAPI(parentCategory, selectedCategoryId);
  }
  ...
  Widget _renderSiblingCategoriesFromLocal(
    ...
    return _renderListCategories(siblingCategories, selectedCategoryId, false);
  }
