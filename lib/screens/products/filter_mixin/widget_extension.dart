...
extension ProductsFilterMixinWidgetExtension on ProductsFilterMixin {
  ...
  Widget renderFilters(
    ...
    return Row(
      children: [
        Expanded(child: title ?? renderFilterTitle(context)),
      ...
      child: Row(
        ...
        children: [
          ...
          for (int i = 0; i < lstSelectedAttribute.length; i++)
            ...List.generate(lstSelectedAttribute.values.elementAt(i).length, (
              index,
            ) {
              final attribute = lstSelectedAttribute.keys.elementAt(i);
              final subAttribute = lstSelectedAttribute.values.elementAt(
                i,
              )[index];
          ...
          for (int i = 0; i < lstSelectedFeature.length; i++)
            ...List.generate(lstSelectedFeature.values.elementAt(i).length, (
              index,
            ) {
              final feature = lstSelectedFeature.keys.elementAt(i);
              final subFeature = lstSelectedFeature.values.elementAt(i)[index];
              final featureName = (feature.name?.capitalize() ?? '').trim();
              final subFeatureName = (subFeature.name ?? '').trim();
              final label = featureName.isNotEmpty && subFeatureName.isNotEmpty
              ...
              return FilterLabel(
                label: label,
                onTap: () {
                  final selectedSubFeature = List<SubFeature>.from(
                    lstSelectedFeature.values.elementAt(i),
                  );

                  selectedSubFeature.removeAt(index);

                  if (selectedSubFeature.isEmpty) {
                    lstSelectedFeature.remove(
                      lstSelectedFeature.keys.elementAt(i),
                    );
                  } else {
                    lstSelectedFeature.update(
                      lstSelectedFeature.keys.elementAt(i),
          ...
          if (tagIds?.isNotEmpty ?? false)
            ...List.generate(tagIds!.length, (index) {
          ...
          if (brandIds?.isNotEmpty ?? false)
            ...List.generate(brandIds!.length, (index) {
