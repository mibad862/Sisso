...
extension ProductsFilterMixinMethodExtension on ProductsFilterMixin {
  ...
  void onFilter({
    ...
    Map<FilterFeature, List<SubFeature>>? features,
    bool fetchProduct = true,
  }) {
    ...
    if (brandIds != null) {
      ...
      final isSameBrand2 = brandIds
          .toSet()
          .difference((this.brandIds ?? []).toSet())
          .isEmpty;
    ...
    if (categoryId != null) {
      ...
      final isSameCate2 = categoryId
          .toSet()
          .difference((categoryIds ?? []).toSet())
          .isEmpty;
      ...
      printLog('[onFilter] ♻️ Reload product list $categoryId');
      if (categoryIds?.length == 1) {
        selectedCategoryName = categoryModel
            .getCategoryById(categoryId.first)
            ?.name;
      }
    ...
    if (!isSameCategory) {
      final futureList = <Future>[
        ...
        _resetFeatures(categoryIds: categoryIds),
      ];
      ...
      final futureList = <Future>[
        ...
        _resetFeatures(categoryIds: categoryIds, brandIds: brandIds),
      ];
      Future.wait(futureList);
    }
  ...
  Map<String, String> getFeatures() {
    ...
    for (var feature in lstSelectedFeature.entries) {
      final terms = <String>[];
      for (var subFeature in feature.value) {
        final subFeatureId = subFeature.id;
        if (subFeatureId != null) {
          terms.add(subFeatureId.toString());
        }
      }
      final featureId = feature.key.id;
      if (featureId != null && terms.isNotEmpty) {
        features[featureId.toString()] = terms.join(',');
      }
    }
  ...
  void applyAttributes(Map<String, dynamic>? attributes) {
    if (attributes != null && attributes.isNotEmpty) {
      for (var attribute in attributes.entries) {
        final attributeValue = filterAttrModel.lstProductAttribute
            ?.firstWhereOrNull((element) => element.slug == attribute.key);

        if (attributeValue != null) {
          lstSelectedAttribute[attributeValue] =
              attributeValue.subAttributes
                  ?.where(
                    (element) =>
                        attribute.value.contains(element.id.toString()),
                  )
                  .toList() ??
              [];
        }
  ...
  void applyFeatures(Map<String, dynamic>? features) {
    if (features != null && features.isNotEmpty) {
      for (var feature in features.entries) {
        final featureValue = filterFeatureModel.listFeature?.firstWhereOrNull(
          (element) => element.id.toString() == feature.key,
        );

        if (featureValue != null) {
          lstSelectedFeature[featureValue] =
              featureValue.subFeatures
                  ?.where(
                    (element) => feature.value.contains(element.id.toString()),
                  )
                  .toList() ??
              [];
        }
  ...
  Future<void> initFilter({ProductConfig? config}) async {
    ...
    final tagIds = config?.tag;

    // Get all brands based on category
    // Reset attributes and tags list based on new category and brand
    final futureList = <Future>[
      if (allowGetBrandByCategory)
        // Do not use [_resetBrands] function because it will reset selected
        // brand which was set before
        brandModel.getBrands(categoryIds: categoryIds)
      else
        // It will always get all brands when initial filter screen
        brandModel.getBrands(),
      if (allowGetTagByCategory)
        _resetTags(categoryIds: categoryIds, brandIds: brandIds)
      else
        // It will always get all tags when initial filter screen
        tagModel.getTags(),
      if (allowGetAttributeByCategory)
        _resetAttributes(categoryIds: categoryIds, brandIds: brandIds)
      else
        // It will always get all attributes when initial filter screen
        filterAttrModel.getFilterAttributes(),
      if (allowGetFeatureByCategory)
        _resetFeatures(categoryIds: categoryIds, brandIds: brandIds)
      else
        // It will always get all features when initial filter screen
        filterFeatureModel.getFilterFeatures(),

      // Load selected brands/tags (including invisible items) to ensure they
      // appear in the model. If not, the selected item will be shown as id only
      // without name.
      brandModel.loadAndAppendBrandsByIds(brandIds),
      tagModel.loadAndAppendTagsByIds(tagIds),
    ];
    ...
    // Selected tags (including invisible tags), set after calling the reset
    // filter because the reset function above may clear the selected tags
    this.tagIds = tagIds;
    ...
    if (attributeId.isNotEmpty && attributeTerm.isNotEmpty) {
      final attribute = filterAttrModel.lstProductAttribute?.firstWhereOrNull(
        (element) =>
      ...
      final listSubAttributes = await filterAttrModel.getSubAttributes(
        attributeId: attribute.id!,
      );

      for (var term in attributeTerm.split(',')) {
        final subAttribute = listSubAttributes?.firstWhereOrNull(
          (element) => element.id == int.parse(term),
        );
    ...
    final featureId = params?.feature ?? '';
    final featureTerm = params?.featureTerm ?? '';
    if (featureId.isNotEmpty && featureTerm.isNotEmpty) {
      final feature = filterFeatureModel.listFeature?.firstWhereOrNull(
        (element) => element.id.toString() == featureId,
      );

      // Don't add feature if this feature is unavailable or invisible
      if (feature == null || !feature.isVisible) return;

      lstSelectedFeature[feature] = [];

      final listSubFeatures = await filterFeatureModel.getSubFeatures(
        featureId: feature.id!,
      );

      for (var term in featureTerm.split(',')) {
        final subFeature = listSubFeatures?.firstWhereOrNull(
          (element) => element.id == int.tryParse(term),
        );
        if (subFeature != null) {
          lstSelectedFeature[feature]!.add(subFeature);
        }
      }
    }
  }
  ...
  Future<void> _resetBrands({List<String>? categoryIds}) async {
    if (allowGetBrandByCategory) {
      ...
      await brandModel.getBrands(categoryIds: categoryIds);
    }
  }
  ...
  Future<void> _resetFeatures({
    ...
    if (allowGetFeatureByCategory) {
      ...
      resetAllSelectedFeature();
      // Get all features by category
      await filterFeatureModel.getFilterFeatures(
        categoryIds: categoryIds?.join(','),
        brandIds: brandIds?.join(','),
      );
    }
