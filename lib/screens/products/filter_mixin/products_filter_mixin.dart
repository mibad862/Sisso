...
mixin ProductsFilterMixin<T extends StatefulWidget> on State<T> {
  ...
  void updateStack(String categoryId, String? parentCategoryId) {
    ...
    if (_stackSelectedCategory.isNotEmpty) {
      ...
      final indexParent = _stackSelectedCategory.indexWhere(
  ...
  String? onToogleCategory({
    ...
    if (categoryId?.isNotEmpty ?? false) {
      ...
      if (level == 0 && jumpStep) {
        final indexLevel0 = _stackSelectedCategory.indexWhere(
          (e) => e.level == 0,
        );
      ...
      if (lastItem == null) {
        ...
        if (level == 1) {
          final indexLevel1 = _stackSelectedCategory.indexWhere(
            (e) => e.level == 1,
          );
          if (indexLevel1 != -1) {
            if (indexLevel1 >= 0) {
  ...
  void updateSelectedSubAttribute({
    ...
    final attribute = filterAttrModel.lstProductAttribute?.firstWhere(
    ...
    if (subAttributes?.indexWhere((element) => element.id == subAttribute.id) ==
      ...
      lstSelectedAttribute[attribute]?.removeWhere(
  ...
  /// List all selected sub features of each selected feature
  Map<FilterFeature, List<SubFeature>> lstSelectedFeature = {};

  void resetAllSelectedFeature() {
    lstSelectedFeature.clear();
  }
  ...
  bool get showLayout => kFilterConfig.showLayout;

  bool get showSort => kFilterConfig.showSort;

  bool get showPriceSlider => kFilterConfig.showPriceSlider;

  bool get showCategory => kFilterConfig.showCategory;

  bool get showListingLocation => kFilterConfig.showListingLocation;

  bool get showAttribute => kFilterConfig.showAttribute;

  bool get showFeature => kFilterConfig.showFeature;

  bool get showTag => kFilterConfig.showTag;

  bool get showBrand => kFilterConfig.showBrand;
  ...
  bool get allowGetTagByCategory =>
      (ServerConfig().isWooPluginSupported || ServerConfig().isPrestashop) &&
  ...
  bool get allowGetFeatureByCategory =>
      (ServerConfig().isPrestashop) &&
      kAdvanceConfig.allowGetDatasByCategoryFilter;
