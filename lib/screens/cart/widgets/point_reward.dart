  ...
  void applyPoints(PointModel pointModel, CartModel cartModel) {
    ...
    if (pointModel.point == null || pointModel.cartPointsRate.isNullOrZero) {
      FlashHelper.errorMessage(
        context,
        message: S.of(context).pointMsgConfigNotFound,
      );
      return;
    }

    if (appliedPoints == 0) {
      FlashHelper.errorMessage(context, message: S.of(context).pointMsgEnter);
      return;
    }
    ...
    if (!applied) {
      ...
      if (appliedPriceDiscount > (cartModel.getTotal() ?? 0)) {
        FlashHelper.errorMessage(
          context,
          message: S.of(context).pointMsgOverMaximumDiscountPoint,
        );
        return;
      }

      setPointDiscount(pointModel, cartModel);
      FlashHelper.message(context, message: S.of(context).pointMsgSuccess);
    } else {
      ...
      FlashHelper.message(context, message: S.of(context).pointMsgRemove);
    }
