...
class GridCategory extends StatefulWidget {
  ...
  const GridCategory({this.scrollController});
...
class _StateGridCategory extends State<GridCategory> with PagingMixin {
  ...
  Widget renderIcon(String categoryId, Map? icons) {
    ...
    if (icon is String) {
      return FluxImage(imageUrl: icon, width: 35, height: 35);
    }
  ...
  Widget build(BuildContext context) {
    var categoriesIcons = Provider.of<AppModel>(
      context,
      listen: true,
    ).categoriesIcons;
    var icons = categoriesIcons;
