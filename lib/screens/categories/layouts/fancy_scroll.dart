...
/// Because initially the basic components of the widget create a size of about
/// 283 and productListItemHeight has a min size of 125.
/// So [_productGridItemHeightOffset] has a size of 158 to ensure the minimum
/// size for the widget is 283. When the user wants to customize the UI
/// (display more items), the size of productListItemHeight needs to be increased.
const _productGridItemHeightOffset = 158;
    ...
    return Consumer<CategoryModel>(
      builder: (context, model, child) {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          model.initSubcategory(
            element,
            fetchData: i < math.min(data.length, _numberOfTabPreload),
          );
        }
          ...
          children: [
            ...
            Expanded(
              child: TabBarView(
                ...
                children: data.map((e) {
                  final detailCategoryModel = model.getDetailCategoryModel(
                    e.id!,
                  );
...
class SyncTabBarWithListView extends StatefulWidget {
  const SyncTabBarWithListView({super.key});
                                    ...
                                    return Column(
                                      ...
                                      children: [
                                        Container(
                                          color: Theme.of(
                                            context,
                                          ).disabledColor.withValueOpacity(0.1),
                                          padding: const EdgeInsets.symmetric(
                                            horizontal: 10,
                                            vertical: 16,
                                          ),
                                          ...
                                          GridView.builder(
                                            gridDelegate:
                                                SliverGridDelegateWithFixedCrossAxisCount(
                                                  ...
                                                  crossAxisSpacing: 6,
                                                  mainAxisSpacing: 6,
                                                  // Add offset to maintain original item height
                                                  mainAxisExtent:
                                                      _productGridItemHeightOffset +
                                                      kProductDetail
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          Container(
            color: Theme.of(context).disabledColor.withValueOpacity(0.1),
            padding: const EdgeInsets.symmetric(vertical: 12),
            child: const Row(
              children: [SizedBox(width: 10), Skeleton(width: 85, height: 25)],
            ),
          ),
          const _GridViewSkeleton(),
        ],
