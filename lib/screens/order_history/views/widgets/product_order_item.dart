...
class _StateProductOrderItem extends BaseScreen<ProductOrderItem>
  ...
  void navigateToProductDetail() async {
    if (product == null) {
      // Try to load product because listing app will load listing product
      // instead of WooComerce product. And order history item cannot be listing
      final productVal = await Services().api.overrideGetProduct(
        widget.product.productId,
      );
      setState(() {
        product = productVal;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.product.addonsOptions.isNotEmpty) {
      for (var element in widget.product.addonsOptions.keys) {
        addonsOptions[element] = Tools.getFileNameFromUrl(
          widget.product.addonsOptions[element]!,
        );
      }
    }
    var deliveryDate;
    if (widget.storeDeliveryDates != null &&
      ...
      var storeIndex = widget.storeDeliveryDates!.indexWhere(
        (element) => element.storeId == widget.product.storeId,
      );
      ...
      children: [
            ...
            children: [
                ...
                child: ClipRRect(
                  ...
                  child: Container(
                    ...
                    child: isLoading
                        ...
                        : ImageResize(url: imageFeatured, fit: BoxFit.cover),
                  ),
                ),
              ),
              const SizedBox(width: 10),
              Flexible(
                child: Column(
                  ...
                  children: [
                    Text(
                      Bidi.stripHtmlIfNeeded(widget.product.name!),
                    ...
                    if (widget.product.addonsOptions.isNotEmpty)
                      Padding(
                        padding: const EdgeInsets.only(top: 8.0),
                        child: HtmlWidget(addonsOptions.values.join(', ')),
                      ),
        ...
        if (widget.orderStatus == OrderStatus.completed &&
          ...
          Padding(
            ...
            child: Reviews(widget.product.productId, showYourRatingOnly: true),
          ),
        const SizedBox(height: 5),
        _buildItemDesc(
          S.of(context).itemTotal,
          PriceTools.getCurrencyFormatted(
            widget.product.subtotal ?? widget.product.total,
            null,
