...
import 'widgets/share_product_data_widget.dart';
...
class ProductDetailScreen extends StatefulWidget {
          ...
          children: <Widget>[
            ...
            if (!isLoading && product != null)
              ListTile(
                ...
                onTap: () {
                  Provider.of<ProductWishListModel>(
                    context,
                    listen: false,
            ...
            if (kProductDetail.allowShareProductData &&
                !isLoading &&
                product != null)
              ListTile(
                title: Text(
                  S.of(modalContext).shareProductData,
                  textAlign: TextAlign.center,
                ),
                onTap: () {
                  Navigator.of(context).pop();
                  _shareProductDataDialog(context, product);
                },
              ),

            // Share feature not supported in Notion. Dont care Strapi because
            // it does not have permalink
            if (dynamicLinkConfig.allowShareLink &&
                productUrl.isNotEmpty &&
                !ServerConfig().isNotion)
              ListTile(
                title: Text(
                  S.of(modalContext).shareProductLink,
                  textAlign: TextAlign.center,
                ),
                onTap: () {
                  Navigator.of(modalContext).pop();
  ...
  static Future<void> _shareProductDataDialog(
    BuildContext context,
    Product? product,
  ) {
    return showDialog<void>(
      ...
      builder: (BuildContext context) {
        return ShareProductDataWidget(product: product);
      },
    );
  }
...
class _ProductDetailPageState extends BaseScreen<ProductDetailScreen>
  ...
  final ProductVariantModel _productVariantModel = ProductVariantModel();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.endOfFrame.then((_) async {
      ...
      if (widget.product is Product) {
        final isListing = widget.product?.isListing != false;

        product = widget.product;
        _productVariantModel.initWithProduct(product!);

        if (!isListing) {
          final check = await _checkProductPermission(product);
          if (check) {
            product = await Services().widget.getProductDetail(
              context,
              product,
            );
            _productVariantModel.updateValues(product: product);
          }
        }
        ...
        final isListing = product?.isListing != false;
  ...
  Future<bool> _checkProductPermission(Product? p) async {
    if (Services().widget.enableMembershipUltimate &&
      ...
      try {
        final check = await Services().api.checkProductPermission(
          p?.id ?? '0',
          cookie,
        );
        setState(() {
        ...
        child: Center(
          child: (_checkingErrorMessage?.isNotEmpty ?? false)
              ? Column(
                  ...
                  children: [
                    Image.asset(kWarning, height: 60),
