...
class _ReviewState extends BaseScreen<ReviewScreen> {
  ...
  void _onNoteChanged() {
    EasyDebounce.debounce(
      'review-screen-order-note',
      const Duration(milliseconds: 200),
      () {
        final trimmedNote = noteController.text.trim();
        final currentNote = cartModel.notes ?? '';
        if (currentNote == trimmedNote) {
          return;
        }
        cartModel.setOrderNotes(trimmedNote);
      },
    );
  }
  ...
  void _prepareCartForCompletion() {
    _statusSubscription = reviewModel.addStatusListener((status) {
      ...
      if (status.isWarning) {
        FlashHelper.informationBar(context, message: message);
        return;
      }
      if (status.isSuccess) {
  ...
  Future<void> _fetchAdditionalFees() async {
    ...
    await feeModel.getFees(cartModel, token, (fees) {
      cartModel.setFees(fees);
      setState(() {});
    });
    await taxModel.getTaxes(cartModel, token, (
      taxesTotal,
      taxes,
      isIncludingTax,
    ) {
  ...
  Widget _buildDesktopLayout() {
    return Column(
      ...
      children: [
        _SectionTitle(title: S.of(context).address, top: 20, bottom: 20),
        const ShippingAddressInfo(useDesktopStyle: true),
        _SectionTitle(title: S.of(context).shippingMethod, top: 20, bottom: 20),
        Services().widget.renderShippingMethodInfo(
          context,
              ...
              builder: (context, model, child) {
                return Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: <Widget>[
                      _ShippingAddressSection(enabledShipping: enabledShipping),
                      Container(
                        height: 1,
                      ...
                      if (kEnableCustomerNote)
                        _CustomerNoteSection(noteController: noteController),
  ...
  List<Widget> _buildProductsList(CartModel model) {
    return model.productsInCart.keys.map((key) {
...
class _SectionTitle extends StatelessWidget {
  ...
  const _SectionTitle({required this.title, this.top = 0, this.bottom = 0});
  ...
  Widget build(BuildContext context) {
    return enabledShipping
        ? ExpansionInfo(
            ...
            children: const <Widget>[ShippingAddressInfo()],
          )
        : const SizedBox();
  }
}
...
class _CustomerNoteSection extends StatelessWidget {
  ...
  const _CustomerNoteSection({required this.noteController});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        const SizedBox(height: 20),
        Text(S.of(context).yourNote, style: const TextStyle(fontSize: 18)),
...
class ShippingAddressInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (kAdvanceConfig.enableWooCheckoutFieldManager) {
      return _WooCheckoutAddressInfo(
        useDesktopStyle: useDesktopStyle,
        wooCheckoutValues: cartModel.wooCheckoutValues,
      );
    }
...
class _WooCheckoutAddressInfo extends StatelessWidget {
  final bool useDesktopStyle;
  final Map<String, dynamic> wooCheckoutValues;

  const _WooCheckoutAddressInfo({
    required this.useDesktopStyle,
    required this.wooCheckoutValues,
  });

  @override
  Widget build(BuildContext context) {
    final billing = _sanitizeMap(wooCheckoutValues['billing']);
    final additional = _sanitizeMap(wooCheckoutValues['additional']);
    final additionalFieldLabels = _sanitizeMap(
      wooCheckoutValues['additionalFieldLabels'],
    );
    final shouldShowShipping = wooCheckoutValues['shouldShowShipping'] == true;
    var shipping = _sanitizeMap(wooCheckoutValues['shipping']);

    if (shouldShowShipping && shipping.isEmpty && billing.isNotEmpty) {
      shipping = billing;
    }

    if (billing.isEmpty && shipping.isEmpty && additional.isEmpty) {
      return const SizedBox();
    }

    final sections = <Widget>[
      if (billing.isNotEmpty)
        _CustomCheckoutFieldsSection(
          title: S.of(context).billingAddress,
          fields: billing,
        ),
      if (shouldShowShipping && shipping.isNotEmpty)
        _CustomCheckoutFieldsSection(
      ...
      if (additional.isNotEmpty)
        _CustomCheckoutFieldsSection(
          ...
          fields: additional,
          fieldLabels: additionalFieldLabels,
        ),
    ];

    if (useDesktopStyle) {
      return Column(children: [...sections, const SizedBox(height: 12)]);
    }
    ...
    return Container(
      ...
      padding: const EdgeInsets.only(left: 10.0, right: 10.0, top: 10.0),
      child: Column(
        children: <Widget>[...sections, const SizedBox(height: 20)],
      ),
    );
  }

  Map<String, String> _sanitizeMap(dynamic rawData) {
    if (rawData is! Map) {
      return const {};
    }
  ...
  String _toDisplayValue(dynamic value) {
    ...
    if (value is List) {
      return value.where((e) => e != null).map((e) => e.toString()).join(', ');
    }
    return value.toString().trim();
  }
}

class _CustomCheckoutFieldsSection extends StatelessWidget {
  final String title;
  final Map<String, String> fields;
  final Map<String, String> fieldLabels;

  const _CustomCheckoutFieldsSection({
    required this.title,
  ...
  String _formatFieldLabel(String key) {
    final exactLabel = fieldLabels[key]?.trim();
    if (exactLabel?.isNotEmpty ?? false) {
      return exactLabel!;
    }
    ...
    final normalizedLabel = fieldLabels[normalized]?.trim();
    if (normalizedLabel?.isNotEmpty ?? false) {
      return normalizedLabel!;
    }

    if (normalized.startsWith('billing_')) {
      normalized = normalized.substring('billing_'.length);
    } else if (normalized.startsWith('shipping_')) {
      normalized = normalized.substring('shipping_'.length);
    }
    ...
    return Column(
      children: [
        ...
        if (address.email?.isNotEmpty ?? false)
          _ItemInfoCheckout(title: S.of(context).email, value: address.email!),
        ...
        if (address.city?.isNotEmpty ?? false)
          _ItemInfoCheckout(title: S.of(context).city, value: address.city!),
