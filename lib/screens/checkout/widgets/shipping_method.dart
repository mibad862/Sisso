...
class _ShippingMethodsState extends State<ShippingMethods> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () async {
      ...
      if (shippingMethods != null &&
          ...
          shippingMethod != null) {
        final index = shippingMethods.indexWhere(
    ...
    return ListenableProvider.value(
      ...
      child: Consumer<ShippingMethodModel>(
        builder: (context, model, child) {
          ...
          final hasError = model.message?.isNotEmpty ?? false;
          final isEmptyMethod = model.shippingMethods?.isEmpty ?? true;

          if (hasError || isEmptyMethod) {
            return Services().renderWidgetWithNetworkConnectState((
              isConnection,
            ) {
              if (isConnection) {
                if (hasError) {
              ...
              return SizedBox(
                ...
                child: Center(
                  child: Text(
                    '⚠️ ${S.of(context).noInternetConnection}',
                    style: const TextStyle(color: kErrorRed),
                  ),
                ),
              );
  ...
  Widget _buildBottom(bool isDesktop) {
    final textButton = kPaymentConfig.enablePreview
    ...
    return CheckoutActionWidget(
      ...
      onTapPrimary: () async {
        ...
        if (shippingMethodModel.shippingMethods?.isNotEmpty ?? false) {
          ...
          final selectedDate = cartModel.selectedDate;
          final requiresTimeSlot =
              selectedDate?.schedule.enableTimeSlot == true &&
              selectedDate?.schedule.isTimeSlotMandatory == true &&
              (selectedDate?.timeSlots.isNotEmpty ?? false);

          final hasSelectedTimeSlot =
              cartModel.selectedTimeSlot?.time?.isNotEmpty ?? false;

          if (requiresTimeSlot && !hasSelectedTimeSlot) {
            Tools.showSnackBar(
              ScaffoldMessenger.of(context),
              S.of(context).pleaseSelectDeliveryTime,
            );
            return;
          }

          Analytics.triggerAddShippingInfo(
            cartModel.shippingMethod?.methodTitle,
            context,
          );
  ...
  Widget buildDeliveryDate() {
    if (!kAdvanceConfig.enableDeliveryDateOnCheckout ||
        !shippingMethodModel.isDeliveryDateEnabled) {
      return const SizedBox();
    }

    final theme = Theme.of(context);

    var deliveryDateLabel = S.of(context).deliveryDate;
    final deliveryDates = shippingMethodModel.deliveryDates;
    if (deliveryDates?.isNotEmpty ?? false) {
      final customLabel = deliveryDates!.first.schedule.deliveryDateLabel;
      if (customLabel.isNotEmpty) {
        deliveryDateLabel = customLabel;
      }
    }
    ...
    Widget deliveryWidget = DateTimePicker(
      onChanged: (DateTime datetime) {
        ...
        orderDeliveryDate.dateString = DateFormat(
          'dd-MM-yyyy HH:mm',
        ).format(datetime);
    ...
    if (shippingMethodModel.deliveryDates?.isNotEmpty ?? false) {
      deliveryWidget = DeliveryCalendar(
        ...
        Padding(
          ...
          child: Text(
            ...
            style: theme.textTheme.bodySmall?.copyWith(
              color: theme.colorScheme.secondary.withValueOpacity(0.7),
            ),
          ),
        ),
        deliveryWidget,
