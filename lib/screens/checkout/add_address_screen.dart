...
class AddAddressArguments {
  ...
  AddAddressArguments({this.address, this.onSaved});
}
...
class _AddAddressScreenState extends State<AddAddressScreen> {
    ...
    try {
      ...
      if (_isEditing) {
        ...
        if (addressIndex != -1) {
          ...
          if (addressNotifier.defaultAddress?.compareFullInfo(
                originalAddress,
              ) ==
              true) {
  ...
  Future<void> _loadCountries() async {
    try {
      ...
      // Auto-populate country field with single country if only one is
      // available and the field is currently empty
      if (loadedCountries.length == 1 &&
          _formManager.controllers[AddressFieldType.country]?.text.isEmpty ==
              true) {
        _formManager.controllers[AddressFieldType.country]?.text =
            loadedCountries.first.code ?? '';
      }
