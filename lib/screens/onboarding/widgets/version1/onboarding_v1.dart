...
class _OnBoardingV1State extends State<OnBoardingV1>
  ...
  List<ContentConfig> getSlides(List<OnBoardingItemConfig> data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var slide = ContentConfig(
        ...
        marginTitle: const EdgeInsets.only(top: 100.0, bottom: 50.0),
        ...
        styleDescription: const TextStyle(fontSize: 15.0),
        foregroundImageFit: BoxFit.fitWidth,
        centerWidget: _renderButtonWidget(data[i].image, isLastItem),
      );
      slides.add(slide);
    }
    return slides;
  ...
  Widget _renderButtonWidget(String imagePath, bool isLastItem) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: <Widget>[
        FluxImage(imageUrl: imagePath, fit: BoxFit.fitWidth),
        if (isLastItem && config.showSignInSignUp && enableAuth)
          Padding(
        ...
        children: [
          SafeArea(
            ...
            child: IntroSlider(
              ...
              renderSkipBtn: MultiLanguageTextButton(
                getText: () => S.of(context).skip,
              ),
              renderDoneBtn: MultiLanguageTextButton(
                getText: () => S.of(context).done,
              ),
              renderPrevBtn: MultiLanguageTextButton(
                getText: () => S.of(context).prev,
              ),
              renderNextBtn: MultiLanguageTextButton(
                getText: () => S.of(context).next,
              ),
              isShowDoneBtn: true,
