...
mixin OnBoardingMixin<T extends StatefulWidget> on ChangeLanguage<T> {
  ...
  final isRequiredLogin = kAuthConfig.isRequired;
  final isEnableRegister = kAuthConfig.enableRegister;
  final isEnableLogin = kAuthConfig.enable;
  ...
  void onTapSignIn() {
    _onHandleFlowAction(
      (ct) => NavigateTools.navigateToLogin(
        ct,
        replacement: true,
  ...
  void _onHandleFlowAction(void Function(BuildContext ct)? onFinish) {
    ...
    if (kIsShowPermissionNotification) {
      Navigator.of(context).pushReplacementNamed(
        ...
        arguments: {
          'onFinish': (BuildContext ctParent) =>
              _openPrivacyTerms(ctParent, (ct) => actionFinish(ct)),
        },
      );
      return;
    }
  ...
  void _openPrivacyTerms(
    BuildContext ctParent,
    void Function(BuildContext ct)? onFinish,
  ) {
    if (kIsShowRequestAgreedPrivacy) {
      Navigator.of(ctParent).pushReplacementNamed(
        RouteList.privacyTerms,
        arguments: {'onFinish': (BuildContext ct) => onFinish?.call(ct)},
      );
