...
mixin SettingNormalMixin on DeleteAccountMixin {
  ...
  SettingUserInfoWidget? get userInfoWidget =>
      hideUser || ServerConfig().isWebView
      ? null
  ...
  /// Currently, `renderVendorOrder` supports WCFM & Dokan,
  /// and `SettingVendorVacationWidget` only supports WCFM.
  ...
  List<Widget> get someExtraMenuForVendorWidget =>
      ...
      ? <Widget>[
          Padding(
            ...
            child: Services().widget.renderVendorOrder(buildContext, cardStyle),
          ),
          Padding(
            padding: EdgeInsets.symmetric(horizontal: itemPadding),
  ...
  List<Widget> get listDynamicItems => List.generate(settings.length, (index) {
    ...
    if (item is! String) {
      ...
      return Padding(
        padding: EdgeInsets.symmetric(horizontal: isTitle ? 0.0 : itemPadding),
