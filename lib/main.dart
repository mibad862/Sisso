...
import 'widgets/overlay/mixin/secure_screen_mixin.dart';
...
void _setupApplication() {
  ...
  /// Force the app to be in edge-to-edge mode to comply with Android 15+.
...
void main() {
  ...
  runZonedGuarded(() async {
    ...
    /// Initialize Google Sign-In (required once before any sign-in calls).
    /// google_sign_in has no Windows implementation, so skip on that platform.
    if (!isWindows) {
      try {
        await GoogleSignIn.instance.initialize();
      } catch (e) {
        printLog('⚠️ GoogleSignIn.initialize() failed: $e');
      }
    }

    /// Block screenshot/recording early (covers splash & onboarding) when the
    /// secure screen feature is enabled for the whole app.
    applyWholeAppSecureScreen();
