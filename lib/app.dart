...
class App extends StatefulWidget {
  ...
  const App({required this.languageCode});
...
class AppState extends State<App>
  ...
  final _filterAttributeModel = FilterAttributeModel();
  final _filterFeatureModel = FilterFeatureModel();
  ...
  void appInitialModules() {
    ...
    Services().initConnectivityService();
    ...
    Future.delayed(const Duration(milliseconds: 200), () {
      ...
      printLog('[AppState] Register Firebase or OneSignal Modules', startTime);
    });
  }
  ...
  ThemeData getTheme({
    ...
    if (isDarkTheme) {
      themeData = buildDarkTheme(
        ...
        fontFamily,
        fontHeader,
        appConfig.settings.useMaterial3,
      );
  ...
  void updateDeviceToken(User? user) {
    if (GmsCheck().isGmsAvailable) {
      ...
      if (user != null && user.username != null) {
        if (!_notificationModel.enable) {
          try {
            Services().api.updateUserInfo({'deviceToken': ''}, user.cookie);
          } catch (err, trace) {
    ...
    final notificationService = injector<NotificationService>();
    notificationService.setUserData(
      userId: user?.id,
      email: user?.email,
    ...
    if (context != null) {
      ...
      unawaited(
        categoryModel.getCategories(
          sortingList: _app?.categories,
          categoryLayout: _app?.categoryLayout,
          remapCategories: _app?.remapCategories,
        ),
      );
    }
  }
  ...
  Future<void> onLoaded(User? user) async {
    ...
    if (isWholesaleEnabled) {
      /// clear cache products  to show correct product price for wholesale
      /// ProductModel notifies listeners via ChangeNotifierProvider, triggering
      /// didChangeDependencies in product screens to reload with updated cookie.
      _product.setProductsList([]);
      _reloadCategories();
    }
    ...
    if (user?.cookie == null) {
      ...
      try {
        final isSuccess = await Services().widget.syncCartFromWebsite(
          user?.cookie,
          cartModel.model,
          context,
        );
    ...
    if (Services().firebase.isEnabled) {
      try {
        if (user?.cookie?.isNotEmpty ?? false) {
          await Services().api.updateUserInfo({
            'deviceToken': '',
          }, user!.cookie);
        }
      } catch (err, trace) {
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider<AppModel>.value(
      value: _app!,
      child: Selector<AppModel, (String, ThemeMode?, AppConfig?, MetaSeoConfig?)>(
        selector: (_, model) => (
          model.langCode,
          model.themeMode,
          model.appConfig,
        ...
        builder: (context, value, child) {
          ...
          if (languageCode.contains('_')) {
            ...
            languageCode = languageCode.substring(
              0,
              languageCode.indexOf(('_')),
            );
          }
            ...
            child: MultiProvider(
              providers: [
                ...
                ChangeNotifierProvider<FilterAttributeModel>.value(
                  value: _filterAttributeModel,
                ),
                ChangeNotifierProvider<FilterFeatureModel>.value(
                  value: _filterFeatureModel,
                ),
                ChangeNotifierProvider(create: (_) => _tagModel),
                ChangeNotifierProvider(
                  create: (_) => cartModel.model,
                  lazy: true,
                ),
                Provider<TaxModel>.value(value: _taxModel),
                ChangeNotifierProvider<FeeModel>(create: (_) => FeeModel()),
                ...
                navigatorObservers: [
                  MyRouteObserver(
                    action: (screenName) {
                      ...
                      return OverlayControlDelegate().emitRoute?.call(
                        screenName,
                      );
                    },
                  ),
                ...
                builder: (buildContext, widget) {
                  ...
                  return botToastBuilder(
                    ...
                    AnnotatedRegion<SystemUiOverlayStyle>(
                      value: getSystemUiOverlay(isDarkTheme),
                      child: LimitTextScaleWidget(
                        maxTextScaleFactor: kMaxTextScale,
                        child: Material(
                          child: Services().manageInternetConnection(
                            Column(
                              children: [
                                Expanded(
                                  child: widget == null
  ...
  void onMessageOpenedApp(FStoreNotificationItem notification) {
    ...
    if (url != null && url.isNotEmpty) {
      unawaited(_handleNotificationOpenLink(url));
      return;
    }

    App.fluxStoreNavigatorKey.currentState?.pushNamed(RouteList.notify);
  }

  Future<void> _handleNotificationOpenLink(String url) async {
    try {
      final isHandled =
          (await Services().dynamicLinkService.handleLink(
            url,
            popToRootBeforeHandle: true,
          )) ==
          true;
      if (!isHandled) {
        await App.fluxStoreNavigatorKey.currentState?.pushNamed(
          RouteList.notify,
        );
      }
    } catch (error, stackTrace) {
      printError(error, null, stackTrace);
      await App.fluxStoreNavigatorKey.currentState?.pushNamed(RouteList.notify);
    }
  }
}
