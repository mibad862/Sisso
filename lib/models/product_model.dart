...
class ProductModel with ChangeNotifier {
  ...
  // Separate recent searches for products and listings
  List<Product>? _recentProductSearch;
  List<Product>? _recentListingSearch;

  ServerConfig get _config => ServerConfig();
  bool get _isListingWoo => _config.isListingWoo;
  bool get _isListingType => _config.isListingType;

  List<Product>? get recentSearch {
    if (_isListingWoo) {
      return productType ? _recentProductSearch : _recentListingSearch;
    }
    if (_isListingType) {
      return _recentListingSearch;
    }
    return _recentProductSearch;
  }

  set recentSearch(List<Product>? value) {
    if (_isListingWoo) {
      productType ? _recentProductSearch = value : _recentListingSearch = value;
    } else if (_isListingType) {
      _recentListingSearch = value;
    } else {
      _recentProductSearch = value;
    }
    notifyListeners();
  }
  ...
  CancelableOperation<List<Product>?>? _cancelLoadProduct;
  ...
  void changeProductVariations(
    ...
    variationsFeatureImages.addAll(
      variations.map((e) => e.imageFeature).whereType<String>(),
    );
    if (notify) {
      notifyListeners();
    }
  ...
  Future<List<Product>?> fetchProductLayout(config, {userId}) async {
  ...
  Future<void> getProductsList({
    ...
    try {
      ...
      } else if (ServerConfig().isShopify &&
          (productsList?.isNotEmpty ?? false)) {
        /// Continue Shopify cursor pagination from the cursor attached to the
        /// last loaded product. This keeps each list's pagination isolated from
        /// other widgets fetching the same collection (e.g. Related Products),
        /// avoiding the "Invalid cursor for current pagination sort" error.
        if (page != 1) {
          cursor = productsList!.last.paginationCursor;
        }
      }

      _cancelLoadProduct = CancelableOperation.fromFuture(
      ...
      if (search?.isNotEmpty ?? false) {
        if (_isListingWoo) {
          if (productType) {
            _recentProductSearch = productsList;
          } else {
            _recentListingSearch = productsList;
          }
        } else if (_isListingType) {
          _recentListingSearch = productsList;
        } else {
  ...
  Future<void> createProduct(
          ...
          for (var i = 0; i < fileImages.length; i++) {
            ...
            await _service.api
                .uploadImage({
                  ...
                  'media_attachment': base64.encode(
                    fileImages[i].readAsBytesSync(),
                  ),
                }, cookie)!
                .then((photo) {
    ...
    await uploadImage().then((_) async {
      var regularPriceMeta = metaData.firstWhereOrNull(
        (e) => e['key'] == '_regular_price',
      );
      await _service.api.createProduct(cookie, {
        'title': nameProduct,
        'product_type': type,
        'content': description,
        'regular_price': regularPriceMeta != null
            ? regularPriceMeta['value']
            : regularPrice,
  ...
  static List<Product> parseWooProductList(
    List? response,
    Map<String, dynamic> config,
  ) {
    var list = <Product>[];
    ...
    if (response != null) {
      ...
      if (include != null && include is List) {
        for (var productID in include) {
          var item = response.firstWhereOrNull(
      ...
      for (var item in products) {
        ...
        final category = config['category']?.toString();
