...
class ShippingMethodModel extends ChangeNotifier {
  ...
  bool _isDeliveryDateEnabled = true;

  List<OrderDeliveryDate>? get deliveryDates => _deliveryDates;
  bool get isDeliveryDateEnabled => _isDeliveryDateEnabled;
  ...
  Future<void> getShippingMethods({
    ...
    try {
      ...
      if (kAdvanceConfig.enableDeliveryDateOnCheckout) {
        final deliveryDates = await getDelivery();
        _isDeliveryDateEnabled = deliveryDates != null;
        _deliveryDates = deliveryDates ?? <OrderDeliveryDate>[];
        if (!_isDeliveryDateEnabled) {
          cartModel.clearOrderDeliveryDate();
        }
      }
      isLoading = false;
      message = null;
