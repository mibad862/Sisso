...
class Order {
  /// Used to identify the order, update, fetch detail, etc.
  String? id;

  /// Used to show on the UI, if not available, use [id] instead
  String? number;
  OrderStatus? status;

  /// In opencart, order_status will be responsed based on language. so I use
  /// this property to show on the UI instead of status property if status is
  /// unknown
  String? orderStatus;
  DateTime? createdAt;
  ...
  Order._fromWooJson(Map parsedJson) {
    try {
      ...
      paymentUrl = parsedJson['payment_url'];
      ...
      shippingMethodTitle =
          parsedJson['shipping_lines'] != null &&
      ...
      if (parsedJson['meta_data'] != null) {
        ...
        for (var item in parsedJson['meta_data']) {
          ...
          if (item['key'] == '_orddd_time_slot' ||
              item['key'] == 'Delivery Time') {
            final timeSlotValue = item['value']?.toString();
            if (timeSlotValue?.trim().isNotEmpty ?? false) {
              deliveryTime = timeSlotValue;
            }
          }
          if (item['key'] == '_wcfmd_delvery_times') {
            ...
            if (deliveryDateMap is Map) {
              deliveryDateMap.forEach((key, value) {
                storeDeliveryDates!.add(
                  StoreDeliveryDate(storeId: key, dateTime: value),
                );
              });
            }
          }
  ...
  void _parseAdditionalInformation(
    dynamic item, {
    required Map<String, String> additionalFieldLabels,
  }) {
    if (!kAdvanceConfig.enableWooCheckoutFieldManager || item is! Map) {
      return;
    }
    ...
    if (key == null ||
        key.isEmpty ||
        !key.startsWith('_') ||
        _reservedMetaKeys.contains(key) ||
        key.startsWith('_wc_order_attribution_')) {
      return;
    }

    final value = _metaToText(item['display_value'] ?? item['value']);
    if (value.isEmpty) {
      return;
    }

    final displayKey = item['display_key']?.toString().trim();
    final mappedLabel = additionalFieldLabels[key]?.trim();
    final title = (displayKey?.isNotEmpty ?? false)
        ? displayKey!
        : (mappedLabel?.isNotEmpty ?? false)
        ? mappedLabel!
        : key
              .replaceFirst('_', '')
              .split('_')
  ...
  String _metaToText(dynamic value) {
    if (value == null) {
      return '';
    }
    if (value is List) {
  ...
  static const Set<String> _reservedMetaKeys = {
    _additionalFieldLabelsMetaKey,
    '_aftership_tracking_items',
    '_orddd_timestamp',
    '_wcfmd_delvery_times',
  };

  static const String _additionalFieldLabelsMetaKey =
      '_fluxstore_additional_field_labels';

  Map<String, String> _extractAdditionalFieldLabels(dynamic rawMetaData) {
    if (rawMetaData is! List) {
      return const {};
    }

    for (final item in rawMetaData) {
      if (item is! Map || item['key'] != _additionalFieldLabelsMetaKey) {
        continue;
      }
  ...
  Order._fromNotionJson(Map parsedJson) {
    try {
      ...
      status = parseOrderStatus(
        NotionDataTools.fromRichTextToText(properties['Status']) ?? 'pending',
      );
      ...
      paymentMethodTitle =
          NotionDataTools.fromRichTextToText(
      ...
      if (dataItems?.isNotEmpty ?? false) {
        ...
        final dataRawListItem = dataTextItemProduct.split(
          NotionDataTools.newlineListData,
        );
          ...
          for (var itemProduct in dataRawListItem) {
            if (itemProduct.isNotEmpty) {
              ...
              for (var item in lineData) {
                ...
                mapDataItem.addAll({dataRaw.first: dataRaw.last});
              }

              final indexProduct = listProducts.indexWhere(
                (element) => element.id == mapDataItem['id'],
              );
      ...
      if (dataRawBilling.isNotEmpty) {
        ...
        for (var item in dataRawBilling) {
          final dataRaw = item.split(':');
          mapDataBilling.addAll({dataRaw.first: dataRaw.last});
        }
      ...
      if (dataRawShipping.isNotEmpty) {
        ...
        for (var item in dataRawShipping) {
          final dataRaw = item.split(':');
          mapDataShipping.addAll({dataRaw.first: dataRaw.last});
        }
        shipping = Address.fromJson(mapDataShipping);
      }

      final dataShippingLines = [];
      final dataRawShippingLines = NotionDataTools.fromRichText(
        properties['ShippingLines'],
      );
        ...
        for (var itemRawShippingLine in dataRawShippingLines!) {
          ...
          if (dataRawLine.isNotEmpty) {
            ...
            for (var item in dataRawLine) {
              final dataRaw = item.split(':');
              mapDataShippingLine.addAll({dataRaw.first: dataRaw.last});
            }
      ...
      deliveryStatus = parseDeliveryStatus(
        properties['delivery_status'] ?? 'pending',
      );
    } catch (e, trace) {
      printLog(e.toString());
      printLog(trace.toString());
  ...
  Order._fromPrestashop(Map parsedJson) {
    try {
      ...
      shippingMethodTitle = parsedJson['shipping_method'] ?? '';
      var totalPaidTaxIncl = double.tryParse(
        parsedJson['total_paid_tax_incl'].toString(),
      );
      var totalPaidTaxExcl = double.tryParse(
        parsedJson['total_paid_tax_excl'].toString(),
      );
  ...
  Map<String, dynamic> toJson(
    ...
    final currencyCode =
        ...
        'USD';
    final currencyRates = cartModel.currencyRates ?? {};
    ...
    for (var key in cartModel.productsInCart.keys) {
        ...
        item['meta_data'] = <Map<String, dynamic>>[
          {
            ...
            'value': (isWholeSaleProduct || isVariationWholesalePrice)
                ? 'yes'
                : 'no',
          },
      ...
      if (cartModel.cartItemMetaDataInCart[key]?.options?.isNotEmpty ?? false) {
        ...
        cartModel.cartItemMetaDataInCart[key]?.options?.forEach((k, v) {
          for (var element in product!.attributes!) {
            if (k == element.keyAttr) {
              Map<String, dynamic>? option = Map.from(
                element.options!.firstWhere(
                  (e) => e['name'] == v,
                  orElse: () => {},
                ),
              );
      ...
      if (cartModel.cartItemMetaDataInCart[key]?.bookingInfo != null) {
        var metaData = item['meta_data'] as List<Map<String, dynamic>>? ?? [];
        var bookingInfo = cartModel.cartItemMetaDataInCart[key]?.bookingInfo
            ?.toJsonAPI();
      ...
      final rentalSelection =
          cartModel.cartItemMetaDataInCart[key]?.rentalInfo?.selection;
      if (rentalSelection != null) {
        final rentalMetaData =
            item['meta_data'] as List<Map<String, dynamic>>? ?? [];
        final rentalDateFormatter = DateFormat('yyyy-MM-dd');
        rentalMetaData.addAll([
          {
            'key': 'wcrp_rental_products_rent_from',
            'value': rentalDateFormatter.format(rentalSelection.start),
          },
          {
            'key': 'wcrp_rental_products_rent_to',
            'value': rentalDateFormatter.format(rentalSelection.end),
          },
        ]);
        item['meta_data'] = rentalMetaData;
      }
      ...
      if (cartModel.cartItemMetaDataInCart[key]?.addonsOptions?.isNotEmpty ??
        ...
        for (var option in options) {
          ...
          itemPrice +=
              (double.tryParse(option.price ?? '0.0') ?? 0) *
      ...
      if (isWalletCart || rentalSelection != null) {
        var itemPrice = cartModel.getProductPrice(key);
        item['subtotal'] = '$itemPrice';
        item['total'] = '$itemPrice';
        ...
        if (productVariantId != null && isVariationWholesalePrice) {
          final itemPrice =
              double.parse(variation!.wholesalePrice!) *
      ...
      if (cartModel
        ...
        for (var addonKey in addonsKeys) {
          ...
          for (var optionKey in optionKeys) {
            ...
            optionValue +=
                ' ${YithAddonsTools.getPriceValueByOption(currencyCode, currencyRates, basePrice, option)}';
            metaData.add({'key': addonKey, 'value': optionValue});
          }
        }
    ...
    try {
      ...
      if (kPaymentConfig.enableAddress &&
          ...
          !kAdvanceConfig.enableWooCheckoutFieldManager) {
      ...
      if (kAdvanceConfig.enableWooCheckoutFieldManager) {
        final billingData = cartModel.wooCheckoutValues['billing'];
        if (billingData is Map && billingData.isNotEmpty) {
          params['billing'] = billingData;
        }

        final shippingData = cartModel.wooCheckoutValues['shipping'];
        if (shippingData is Map && shippingData.isNotEmpty) {
          params['shipping'] = shippingData;
        }
      }
      ...
      if (cartModel.fees.isNotEmpty) {
        for (var fee in cartModel.fees) {
          feeLines.add({
            'name': fee.name,
            'tax_status': 'taxable',
            'total': fee.total,
      ...
      if (kAdvanceConfig.enableDeliveryDateOnCheckout) {
        ...
        if (cartModel.selectedDate != null) {
          ...
          final selectedTime = cartModel.selectedTimeSlot?.time;
          if (selectedTime?.isNotEmpty ?? false) {
            metaData.addAll([
              {'key': 'Delivery Time', 'value': selectedTime},
              {'key': '_orddd_time_slot', 'value': selectedTime},
            ]);
          }
        }
      ...
      final additionalData = cartModel.wooCheckoutValues['additional'];
      if (kAdvanceConfig.enableWooCheckoutFieldManager &&
        ...
        final additionalFieldLabels =
            cartModel.wooCheckoutValues['additionalFieldLabels'];
        if (additionalFieldLabels is Map && additionalFieldLabels.isNotEmpty) {
          metaData.add({
            'key': _additionalFieldLabelsMetaKey,
            'value': {
              for (final entry in additionalFieldLabels.entries)
                entry.key.toString(): entry.value.toString(),
            },
          });
        }
  ...
  Order._fromHaravan(Map parsedJson) {
    try {
      ...
      total = parsedJson['total_price'] != null
          ? num.tryParse(
              parsedJson['total_price']?.toString() ?? '',
            )?.toDouble()
          : 0.0;
      totalTax = parsedJson['total_tax'] != null
      ...
      if (fulfillments != null &&
        ...
        deliveryStatus = parseDeliveryStatus(
          fulfillments[0]['carrier_status_code'],
        );
  ...
  Map<String, dynamic> toServerlessJson(
    CartModel cartModel,
    userId,
    paid, {
    bool isGuest = false,
  ...
  Map<String, dynamic> toNotiontoJson(
    int idNotionOrder,
    CartModel cartModel,
    String userId,
    String nameUser,
    ...
    return {
      'Name': NotionDataTools.toTitle(
        '$nameUser($userId)-OrderNo.#$idNotionOrder',
      ),
      'Customer': NotionDataTools.toRelation([userId]),
      'PaymentMethod': NotionDataTools.toRichText(params['payment_method']),
      ...deliveryDate,
      'PaymentMethodTitle': NotionDataTools.toRichText(
      ...
      'ShippingTotal': NotionDataTools.toNumber(
        cartModel.shippingMethod?.cost ?? 0,
      ),
  ...
  Map<String, dynamic> toPrestashopJson(CartModel cartModel, userId, paid) {
    ...
    final params = {'line_items': lineItems};
  ...
  String displayTitle(BuildContext context) {
    switch (this) {
      ...
      case OrderStatus.pending:
        return S.of(context).pendingPayment;
