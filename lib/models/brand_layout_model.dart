...
class BrandLayoutModel extends ChangeNotifier {
  ...
  Future<void>? _getBrandsFuture;
  Future<void>? _loadAllBrandsFuture;
  ...
  Future<void> getBrands() async {
    final pendingRequest = _getBrandsFuture;
    if (pendingRequest != null) {
      return pendingRequest;
    }

    final request = _getBrandsInternal();
    _getBrandsFuture = request;

    try {
      await request;
    } finally {
      if (identical(_getBrandsFuture, request)) {
        _getBrandsFuture = null;
      }
    }
  }

  Future<void> _getBrandsInternal() async {
    try {
  ...
  Future<void> loadAllBrands() async {
    final pendingRequest = _loadAllBrandsFuture;
    if (pendingRequest != null) {
      return pendingRequest;
    }

    final request = _loadAllBrandsInternal();
    _loadAllBrandsFuture = request;

    try {
      await request;
    } finally {
      if (identical(_loadAllBrandsFuture, request)) {
        _loadAllBrandsFuture = null;
      }
    }
  }

  Future<void> _loadAllBrandsInternal() async {
    if (_brands.isEmpty) {
      await getBrands();
    }

    while (!_isDisposed && !_isEnded) {
      final batch = await loadMoreBrands();
      if (batch.isEmpty) {
        break;
      }
    }
