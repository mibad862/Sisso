import 'dart:async';

import 'package:collection/collection.dart' show IterableExtension;
import 'package:flutter/material.dart';
import 'package:flux_localization/flux_localization.dart';

import '../../common/config.dart';
import '../../common/config/models/b2bking_config.dart';
import '../../common/constants.dart';
import '../../common/tools/rental_tools.dart';
import '../../common/tools/yith_addons_tools.dart';
import '../../data/boxes.dart';
import '../../services/index.dart';
import '../entities/b2bking/tiered_price.dart';
import '../entities/index.dart'
    show Product, ProductVariation, Tax, PWGiftExt, YithAddonsOption, Fee;
import '../entities/product_addons.dart';
import 'cart_base.dart';
import 'cart_item_meta_data.dart';
import 'mixin/index.dart';

class CartModelWoo
    with
        ChangeNotifier,
        CartMixin,
        MagentoMixin,
        AddressMixin,
        LocalMixin,
        CurrencyMixin,
        CouponMixin,
        VendorMixin,
        OrderDeliveryMixin
    implements CartModel {
  static final CartModelWoo _instance = CartModelWoo._internal();

  factory CartModelWoo() => _instance;

  CartModelWoo._internal();

  @override
  Future<void> initData() async {
    resetValues();
    await getAddress();
    getCurrency();
  }

  @override
  double getTotal() {
    var subtotal = getSubTotal() ?? 1.0;

    if (couponObj != null) {
      subtotal -= getCouponCost();
    }

    if (kPaymentConfig.enableShipping) {
      subtotal += getShippingCost(
        includeTax: shippingMethod?.isIncludingTax ?? true,
      )!;
    }

    if (taxes.isNotEmpty && !isIncludingTax) {
      subtotal += taxesTotal;
    }

    if (fees.isNotEmpty) {
      for (var fee in fees) {
        subtotal += double.tryParse('${fee.total}') ?? 0.0;
      }
    }

    subtotal -= rewardTotal;

    subtotal -= walletAmount;

    subtotal += getCODExtraFee();

    subtotal -= loyaltyCoupon?.value ?? 0;

    if (subtotal < 0.0) {
      subtotal = 0.0;
    }

    return subtotal;
  }

  @override
  double? getPaymentTotal() {
    if (wooSmartCod?.isRiskFreeEnabled == true) {
      return wooSmartCod?.userAdvanceAmount;
    }
    return super.getPaymentTotal();
  }

  @override
  double getProductPrice(id) {
    var productId = Product.cleanProductID(id);
    if (item[productId] != null) {
      if (item[productId]!.type == 'subscription') {
        final signUpFee = item[productId]!.metaData.firstWhere(
          (element) => element['key'] == '_subscription_sign_up_fee',
          orElse: () => {},
        )['value'];
        if ((signUpFee?.isNotEmpty ?? false) &&
            productsInCart[id] != null &&
            double.parse(signUpFee!) > 0) {
          return double.parse(signUpFee!) * productsInCart[id]!;
        }
      }
    }

    if (cartItemMetaDataInCart[id]?.rentalInfo != null) {
      final rentalTotal = RentalTools.getCartTotal(
        cartItemMetaDataInCart[id]?.rentalInfo,
        productsInCart[id],
      );
      if (rentalTotal != null) {
        return rentalTotal;
      }
    }

    if (cartItemMetaDataInCart[id]?.selectedTieredPrice != null) {
      return double.parse(
            cartItemMetaDataInCart[id]?.selectedTieredPrice?.price
                    ?.toString() ??
                '0',
          ) *
          productsInCart[id]!;
    }

    if (cartItemMetaDataInCart[id]?.selectedYithOptions != null) {
      var basePrice =
          cartItemMetaDataInCart[id]!.variation?.valuePrice ??
          item[productId]?.valuePrice ??
          0;
      return YithAddonsTools.getSelectedYithOptionsPrice(
            basePrice,
            cartItemMetaDataInCart[id]!.selectedYithOptions!,
            productsInCart[id]!,
          ) +
          basePrice * productsInCart[id]!;
    }

    return super.getProductPrice(id);
  }

  @override
  bool isEnabledShipping() {
    List ids = productsInCart.keys.where((id) {
      var productId = Product.cleanProductID(id);
      return item[productId]!.type == 'subscription' ||
          item[productId]!.type == 'variable-subscription';
    }).toList();
    if (ids.length == productsInCart.keys.length || isWalletCart()) {
      return false;
    }
    return super.isEnabledShipping();
  }

  @override
  double getItemTotal({
    ProductVariation? productVariation,
    Product? product,
    int quantity = 1,
  }) {
    var subtotal = double.parse(product!.price!) * quantity;
    if (productVariation != null) {
      subtotal = double.parse(productVariation.price!) * quantity;
    } else {
      subtotal = double.parse(product.price!) * quantity;
    }
    if (product.selectedOptions?.isNotEmpty ?? false) {
      subtotal += product.getProductOptionsPrice(quantity);
    }
    return subtotal;
  }

  @override
  String updateQuantity(Product product, String key, int quantity) {
    final isClosed =
        (product.store != null && product.store!.storeHour != null) &&
        (product.store!.storeHour!.isDisablePurchase! &&
            !product.store!.storeHour!.isOpen());
    final isOnVacation =
        (product.store != null && product.store!.vacationSettings != null) &&
        (product.store!.vacationSettings!.vacationMode &&
            !product.store!.vacationSettings!.isOpen() &&
            product.store!.vacationSettings!.disableVacationPurchase);

    if (isClosed) {
      return S.current.storeClosed;
    }
    if (isOnVacation) {
      return S.current.onVacation;
    }
    var message = '';
    ProductVariation? variation;

    if (key.contains('-')) {
      variation = getProductVariationById(key);
    }

    final allowBackorder = variation != null
        ? (variation.backordersAllowed ?? false)
        : product.backordersAllowed;

    message = checkMinMaxQuantity(
      product: product,
      key: key,
      totalQuantity: quantity,
      allowBackorder: allowBackorder,
      variation: variation,
    );

    if (message.isEmpty) {
      updateSelectedTieredPrice(key, quantity);
      updateQuantityCartLocal(key: key, quantity: quantity);
      notifyListeners();
    }

    updateDiscount(onFinish: notifyListeners);
    notifyListeners();

    Services().widget.syncCartToWebsite(this);

    return message;
  }

  // Removes an item from the cart.
  @override
  void removeItemFromCart(String key) {
    if (productsInCart.containsKey(key)) {
      productsInCart.remove(key);
      cartItemMetaDataInCart.remove(key);

      /// In case there are multiple item with same product ID.
      /// Example: [1234+Small, 1234+Medium]
      var shouldRemove = true;
      var productId = Product.cleanProductID(key);
      for (var inCartKey in productsInCart.keys) {
        var productIdInCart = Product.cleanProductID(inCartKey);
        if (productIdInCart == productId) {
          shouldRemove = false;
          break;
        }
      }
      if (shouldRemove) {
        item.remove(productId);
      }

      removeProductLocal(key);
    }

    updateDiscount(onFinish: notifyListeners);
    notifyListeners();

    Services().widget.syncCartToWebsite(this);
  }

  // Removes an item from the cart.
  @override
  void removeItemFromProductId(String productId) {
    productsInCart.removeWhere((key, value) => key.contains(productId));
    removeProductLocal(productId);

    updateDiscount(onFinish: notifyListeners);
    notifyListeners();

    Services().widget.syncCartToWebsite(this);
  }

  // Removes everything from the cart.
  @override
  Future<void> clearCart({isSaveRemote = true, isSaveLocal = true}) async {
    if (isWalletCart()) {
      getCurrency();
    }
    if (isSaveLocal) {
      await clearCartLocal();
    }
    productsInCart.clear();
    item.clear();
    cartItemMetaDataInCart.clear();
    shippingMethod = null;
    paymentMethod = null;
    loyaltyCoupon = null;
    wooSmartCod = null;
    resetCoupon();
    notes = null;
    rewardTotal = 0;
    walletAmount = 0;
    taxesTotal = 0;
    isIncludingTax = false;
    taxes = [];
    fees = [];
    notifyListeners();

    if (isSaveRemote) {
      unawaited(Services().widget.syncCartToWebsite(this));
    }
  }

  @override
  void setOrderNotes(String note) {
    notes = note;
    notifyListeners();
  }

  String _validateProductComponent(
    Product? product,
    CartItemMetaData? cartItemMetaData,
  ) {
    var errMsg = '';
    if (product?.isCompositeProduct == true) {
      if (cartItemMetaData?.selectedComponents == null) {
        errMsg = S.current.pleaseSelectRequiredOptions;
      } else {
        product?.components?.forEach((e) {
          if (e.required == true &&
              cartItemMetaData?.selectedComponents?[e.id] == null) {
            errMsg = '${e.name ?? ''} is required';
            return;
          }
        });
      }
    }
    return errMsg;
  }

  @override
  FutureOr<(bool, String)> addProductToCart({
    required BuildContext context,
    required Product product,
    int quantity = 1,
    Function? notify,
    isSaveLocal = true,
    isSaveRemote = true,
    CartItemMetaData? cartItemMetaData,
  }) async {
    if (isWalletCart()) {
      await clearCart(isSaveRemote: isSaveRemote);
    }

    var message = '';

    message = _validateProductComponent(product, cartItemMetaData);
    if (message.isNotEmpty) return (false, message);

    if (product.store != null) {
      final isClosed =
          (product.store!.storeHour != null) &&
          (product.store!.storeHour!.isDisablePurchase! &&
              !product.store!.storeHour!.isOpen());
      if (isClosed) {
        message = S.of(context).storeClosed;
        return (false, message);
      }

      if (product.store?.vacationSettings != null) {
        final isOnVacation =
            (product.store!.vacationSettings!.vacationMode &&
            !product.store!.vacationSettings!.isOpen() &&
            product.store!.vacationSettings!.disableVacationPurchase);
        if (isOnVacation) {
          message = S.of(context).onVacation;
          return (false, message);
        }
      }
    }

    var key = product.id.toString();
    if (cartItemMetaData?.variation != null) {
      if (cartItemMetaData?.variation?.id != null) {
        key += '-${cartItemMetaData?.variation?.id}';
      }
      if (cartItemMetaData?.options != null) {
        for (var option in cartItemMetaData!.options!.keys) {
          key += '-$option${cartItemMetaData.options![option]}';
        }
      }
    } else if (product.bookingInfo != null) {
      key += '-${UniqueKey().toString()}';
    }
    if (cartItemMetaData?.selectedComponents != null) {
      for (var selectedComponentKey
          in (cartItemMetaData?.selectedComponents?.keys.toList() ?? [])) {
        var selected =
            cartItemMetaData?.selectedComponents?[selectedComponentKey];
        if (selected != null) {
          key += '-$selectedComponentKey-${selected.product.id}';
          if (selected.variant?.id?.isNotEmpty ?? false) {
            key += '-${selected.variant?.id}';
          }
        }
      }
    }

    final allowBackorder = cartItemMetaData?.variation != null
        ? (cartItemMetaData?.variation?.backordersAllowed ?? false)
        : product.backordersAllowed;

    try {
      /// Product variations.
      if (product.isVariableProduct) {
        /// Loading attributes & variants.
        if (cartItemMetaData?.variation == null) {
          if (product.attributes != null &&
              product.attributes?.length == cartItemMetaData?.options?.length) {
            message = S.of(context).outOfStock;
          } else {
            message = S.of(context).pleaseSelectAllAttributes;
          }
          return (false, message);
        }

        /// Not selected all attributes.
        if ((cartItemMetaData?.options?.isEmpty ?? true) ||
            cartItemMetaData!.options!.values.contains(null)) {
          message = S.of(context).pleaseSelectAllAttributes;
          return (false, message);
        }
      }

      /// Product addons.
      if (product.addOns?.isNotEmpty ?? false) {
        for (var addOns in product.addOns!) {
          if (addOns.required ?? false) {
            /// For customer-defined price, text & upload type, label is entered by user.
            /// So we need to check using addon name.
            final requiredOptions = addOns.options!.map((e) {
              if (e.type?.isEnteredByUser ?? false) {
                return e.parent;
              }
              return e.label;
            }).toList();
            final check = product.selectedOptions?.firstWhereOrNull(
              (option) => requiredOptions.contains(
                (option.type?.isEnteredByUser ?? false)
                    ? option.parent
                    : option.label,
              ),
            );
            if (check == null) {
              message = S.of(context).pleaseSelectRequiredOptions;
              return (false, message);
            }
          }
        }
      }

      if (product.selectedOptions?.isNotEmpty ?? false) {
        key += '+${product.selectedOptions!.map((e) => e.label).join('+')}';
      }
      final pwGiftCardInfo = cartItemMetaData?.pwGiftCardInfo;
      if (product.isPWGiftCardProduct) {
        key += '+${pwGiftCardInfo?.key}';
        if (!(pwGiftCardInfo?.isValid ?? false)) {
          message = S.of(context).pleaseInputFillAllFields;
          return (false, message);
        }
        var emails = pwGiftCardInfo?.to.split(',') ?? [];
        quantity = emails.length;
        for (var email in emails) {
          if (!email.isEmail) {
            message = S.of(context).errorEmailFormat;
            return (false, message);
          }
        }
      }
      if (cartItemMetaData?.selectedYithOptions?.isNotEmpty ?? false) {
        var options = <YithAddonsOption>[];
        for (var item in cartItemMetaData!.selectedYithOptions!.values) {
          options.addAll(item.values);
        }
        key +=
            '+${options.map((e) => e.inputValue?.toString() ?? e.label ?? '').join('+')}';
      }

      if (cartItemMetaData?.rentalInfo != null) {
        key +=
            '-${cartItemMetaData?.rentalInfo?.selection?.start.toIso8601String()}_${cartItemMetaData?.rentalInfo?.selection?.end.toIso8601String()}';
      }
      //Check product's quantity before adding to cart
      var existedItem = productsInCart.containsKey(key);
      message = checkMinMaxQuantity(
        product: product,
        key: key,
        totalQuantity: !existedItem
            ? quantity
            : productsInCart[key]! + quantity,
        allowBackorder: allowBackorder,
        variation: cartItemMetaData?.variation,
      );

      if (message.isEmpty) {
        item[product.id] = product;
        cartItemMetaDataInCart[key] =
            (cartItemMetaData ?? const CartItemMetaData()).copyWith(
              bookingInfo: product.bookingInfo,
              addonsOptions: product.selectedOptions
                  ?.map(AddonsOption.copy)
                  .toList(),
            );
        if (existedItem) {
          updateSelectedTieredPrice(key, productsInCart[key] ?? quantity);
        }
        if (isSaveLocal) {
          saveCartToLocal(
            key,
            product: product,
            quantity: productsInCart[key],
            cartItemMetaData: cartItemMetaDataInCart[key],
          );
        }
      }

      unawaited(updateDiscount(onFinish: notifyListeners));
      notifyListeners();

      if (isSaveRemote) {
        unawaited(Services().widget.syncCartToWebsite(this));
      }
    } catch (err) {
      if (err is ArgumentError) {
        message = S.of(context).pleaseSelectAllAttributes;
        return (false, message);
      }
    }
    return (message.isEmpty, message);
  }

  @override
  void setRewardTotal(double total) {
    rewardTotal = total;
    notifyListeners();
  }

  @override
  void setLoyaltyCoupon(data) {
    loyaltyCoupon = data;
    notifyListeners();
  }

  double getShippingVendorCost({bool includeTax = false}) {
    var sum = 0.0;

    for (var element in selectedShippingMethods) {
      sum += element.shippingMethods[0].cost ?? 0.0;
      if (includeTax) {
        sum += element.shippingMethods[0].shippingTax ?? 0.0;
      }
    }
    return sum;
  }

  @override
  double? getShippingCost({bool includeTax = false}) {
    var isMultiVendor = ServerConfig().isVendorType();
    return isMultiVendor
        ? getShippingVendorCost(includeTax: includeTax)
        : super.getShippingCost(includeTax: includeTax);
  }

  @override
  void loadSavedCoupon() {
    savedCoupon = UserBox().savedCoupon;
    notifyListeners();
  }

  @override
  void setWalletAmount(double total) {
    super.setWalletAmount(total);
    notifyListeners();
  }

  @override
  void setWooSmartCod(data) {
    wooSmartCod = data;
    notifyListeners();
  }

  bool isTopUpProduct(Product? product) {
    return product != null && product.isTopUpProduct();
  }

  @override
  bool isWalletCart() {
    var isWallet = false;
    for (var key in productsInCart.keys) {
      var productId = Product.cleanProductID(key);
      var product = item[productId];
      if (isTopUpProduct(product)) {
        isWallet = true;
        break;
      }
    }

    return isWallet;
  }

  @override
  bool isB2BKingCart() {
    if (kAdvanceConfig.b2bKingConfig.enabled &&
        kAdvanceConfig.b2bKingConfig.guestAccessRestriction ==
            GuestAccessRestriction.replacePricesQuote &&
        user == null) {
      return true;
    }

    var isB2BKing = false;
    for (var item in cartItemMetaDataInCart.values) {
      if (item?.selectedTieredPrice != null) {
        isB2BKing = true;
        break;
      }
    }

    return isB2BKing;
  }

  @override
  void setTaxInfo(List<Tax> taxes, double taxesTotal, bool isIncludingTax) {
    super.setTaxInfo(taxes, taxesTotal, isIncludingTax);
    notifyListeners();
  }

  @override
  void setFees(List<Fee> fees) {
    super.setFees(fees);
    notifyListeners();
  }

  @override
  double getProductAddonsPrice(String id) {
    var addOns = cartItemMetaDataInCart[id]?.addonsOptions;
    var price = 0.0;
    if (addOns == null) {
      return 0.0;
    }
    for (var option in addOns) {
      var quantity = productsInCart[id] ?? 0;
      var optionPrice = (double.tryParse(option.price ?? '0.0') ?? 0.0);
      price += optionPrice * quantity;
    }
    return price;
  }

  @override
  void updateProduct(String productId, Product? product) {
    super.updateProduct(productId, product);
    notifyListeners();
  }

  @override
  void updateProductVariant(
    String productId,
    ProductVariation? productVariant,
  ) {
    super.updateProductVariant(productId, productVariant);
    notifyListeners();
  }

  @override
  void updateStateCheckoutButton() {
    super.updateStateCheckoutButton();
    notifyListeners();
  }

  /// Support for min-max-quantity for Woo plugin
  String checkMinMaxQuantity({
    required Product product,
    required String key,
    required int totalQuantity,
    ProductVariation? variation,
    bool allowBackorder = false,
  }) {
    String checkQuantityGroupOf() {
      final step = product.quantityStep;
      if (step != null && totalQuantity % step != 0) {
        return '${product.name} ${S.current.mustBeBoughtInGroupsOf(step)}. ${S.current.pleaseIncreaseOrDecreaseTheQuantity}';
      }
      productsInCart[key] = totalQuantity;
      return '';
    }

    var stockQuantity = variation == null
        ? product.stockQuantity
        : variation.stockQuantity;
    var manageStock = variation == null
        ? product.manageStock
        : variation.manageStock;
    var minQuantity = product.minQuantity;
    var maxQuantity = product.maxQuantity;
    final maxAllowQuantity = kCartDetail.maxAllowQuantity;
    final useMaxAllowQuantity =
        !allowBackorder &&
        maxQuantity != null &&
        maxQuantity > maxAllowQuantity;

    if (maxQuantity == null || useMaxAllowQuantity) {
      maxQuantity = maxAllowQuantity;
    }

    if (!manageStock || allowBackorder || totalQuantity <= stockQuantity!) {
      if (minQuantity == null) {
        if (totalQuantity > maxQuantity && !allowBackorder) {
          return '${S.current.youCanOnlyPurchase} $maxQuantity ${S.current.forThisProduct}';
        }
        return checkQuantityGroupOf();
      } else if (totalQuantity >= minQuantity && totalQuantity <= maxQuantity) {
        return checkQuantityGroupOf();
      } else {
        if (totalQuantity < minQuantity) {
          return '${S.current.minimumQuantityIs} $minQuantity';
        }
        if (totalQuantity > maxQuantity && !allowBackorder) {
          return '${S.current.youCanOnlyPurchase} $maxQuantity ${S.current.forThisProduct}';
        }
        return checkQuantityGroupOf();
      }
    } else {
      return '${S.current.currentlyWeOnlyHave} $stockQuantity ${S.current.ofThisProduct}';
    }
  }

  void updateSelectedTieredPrice(String key, int quantity) {
    var cartItemMetaData = cartItemMetaDataInCart[key];
    //update the correct selectedTieredPrice when change quantity
    if (cartItemMetaData?.tieredPrices?.isNotEmpty ?? false) {
      var selectedTieredPrice = cartItemMetaData?.tieredPrices?.reversed
          .firstWhereOrNull((e) => e.quantity <= quantity);
      if (selectedTieredPrice != null) {
        cartItemMetaDataInCart[key] = cartItemMetaData?.copyWith(
          selectedTieredPrice: selectedTieredPrice,
        );
      } else {
        cartItemMetaDataInCart[key] = cartItemMetaData
            ?.removeSelectedTieredPrice();
      }
    }
  }

  Future<void> refreshTieredPricesForCartItems({
    Iterable<String>? keys,
    String? token,
  }) async {
    if (!kAdvanceConfig.b2bKingConfig.enabled) {
      return;
    }

    final cartKeys = keys?.toList() ?? productsInCart.keys.toList();

    final entriesByProductId = <String, List<({String key, int quantity})>>{};
    final productById = <String, Product>{};

    for (final key in cartKeys) {
      final quantity = productsInCart[key];
      if (quantity == null) continue;

      final productId = Product.cleanProductID(key);
      final product = item[productId];
      final cartItemMetaData = cartItemMetaDataInCart[key];
      if (product == null || cartItemMetaData == null) continue;

      final shouldRefresh =
          (cartItemMetaData.selectedTieredPrice != null) ||
          (cartItemMetaData.tieredPrices?.isNotEmpty ?? false) ||
          product.b2bTieredPrices.isNotEmpty;
      if (!shouldRefresh) continue;

      productById[productId] = product;
      entriesByProductId.putIfAbsent(productId, () => []).add((
        key: key,
        quantity: quantity,
      ));
    }

    if (entriesByProductId.isEmpty) return;

    final tieredPricesByProductId = <String, List<B2BKingTieredPrice>>{};
    final productsToFetch = <String>[];
    entriesByProductId.forEach((productId, _) {
      final cached = productById[productId]!.b2bTieredPrices;
      if (cached.isNotEmpty) {
        tieredPricesByProductId[productId] = cached.cast<B2BKingTieredPrice>();
      } else {
        productsToFetch.add(productId);
      }
    });

    for (final productId in entriesByProductId.keys) {
      final tieredPrices = tieredPricesByProductId[productId];
      if (tieredPrices == null) continue;

      final product = productById[productId]!;
      final entries = entriesByProductId[productId]!;

      for (final entry in entries) {
        final cartItemMetaData = cartItemMetaDataInCart[entry.key];
        if (cartItemMetaData == null) continue;

        final selectedTieredPrice = tieredPrices.reversed.firstWhereOrNull(
          (e) => e.quantity <= entry.quantity,
        );

        cartItemMetaDataInCart[entry.key] = cartItemMetaData
            .updateTieredPricing(
              tieredPrices: tieredPrices.isEmpty ? null : tieredPrices,
              selectedTieredPrice: selectedTieredPrice,
            );

        saveCartToLocal(
          entry.key,
          product: product,
          quantity: entry.quantity,
          cartItemMetaData: cartItemMetaDataInCart[entry.key],
        );
      }
    }
  }
}
