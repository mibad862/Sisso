...
mixin CartMixin {
  ...
  /// The currency code for the cart and checkout process. It may be different
  /// from the currency code of AppModel. For example, in the case of a wallet
  /// cart, the currency code is set to the default currency code.
  String? currencyCode;
  ...
  Map<String, dynamic> wooCheckoutValues = {};
  ...
  double getProductPrice(id) {
    if (_hasProductVariation(id)) {
      ...
      final quantity = productsInCart[id] ?? 0;
      return (variation?.valuePrice ?? 0.0) * quantity;
    } else {
      var productId = Product.cleanProductID(id);
      final product = item[productId];
      final isSale = product?.isOnSaleActive ?? false;

      var price = PriceTools.getPriceProductValue(
        product,
        bookingInfo: cartItemMetaDataInCart[id]?.bookingInfo,
        onSale: isSale,
      );
  ...
  void addWalletProductToCart({required Product product, int quantity = 1}) {
    final key = product.id.toString();
    item[product.id] = product;
    productsInCart[key] = quantity;

    currencyCode = kCurrencyConfig.defaultCurrency?.currencyCode;
  }
  ...
  void updateProductVariant(
    ...
    cartItemMetaDataInCart[productId] = cartItemMetaDataInCart[productId]
        ?.copyWith(variation: productVariant);
  }
  ...
  void updateStateCheckoutButton() {
    ...
    for (var key in productsInCart.keys) {
      ...
      var totalQuantity = variation != null
          ? (variation.stockQuantity ?? maxAllowQuantity)
  ...
  void setWooCheckoutValues(Map<String, dynamic> checkoutValues) {
    wooCheckoutValues = checkoutValues;
  }
}
