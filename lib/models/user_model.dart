...
class UserModel with ChangeNotifier {
  ...
  Future<String?> submitForgotPassword({
    ...
    return await _service.api.submitForgotPassword(
      forgotPwLink: forgotPwLink,
      data: data,
    );
  }
        ...
        case apple.AuthorizationStatus.authorized:
          {
            ...
            Analytics.triggerLogin(method: 'apple');
            success!(user);
  ...
  Future<void> loginFirebaseSMS({
    ...
    try {
      ...
      await _saveUser(user);
      Analytics.triggerLogin(method: 'sms');
      success(user);
    ...
    try {
      ...
      switch (result.status) {
        case LoginStatus.success:
          ...
          await _saveUser(user);
          Analytics.triggerLogin(method: 'facebook');
          success!(user);
          break;
  ...
  Future<void> loginGoogle({Function? success, Function? fail, context}) async {
    try {
      ...
      user = await _authProviderService.loginGoogle(
        token: authz.accessToken,
        email: account.email,
        displayName: account.displayName,
        picture: account.photoUrl,
      );
      await _saveUser(user);
      Analytics.triggerLogin(method: 'google');
      success!(user);
      notifyListeners();
      ...
      if (e.code == GoogleSignInExceptionCode.canceled) {
        ...
        fail!(S.of(context).loginErrorServiceProvider(e.toString()));
      }
    } catch (err, trace) {
  ...
  Future<void> loginWithCookie(
    ...
    try {
      ...
      if (user == null) {
        ...
        final hasInfoUser = cookies.any(
          (element) =>
              [
                'pro_loyalty_api_session',
                'customer_sig',
      ...
      await _saveUser(user);
      Analytics.triggerLogin(method: 'cookie');
      success?.call(user!);
      loading = false;
      notifyListeners();
    } catch (err) {
  ...
  Future<void> createUser({
    ...
    try {
      ...
      Future<void> loginAccountFirebase() async {
      ...
      await _saveUser(user);
      Analytics.triggerRegister(method: 'email');
      success(user);
    ...
    try {
      ...
      if (_isFirebaseServerless == false) {
        ...
        if (kLoyaltyConfig.enabled) {
          //Loyalty should verify whether the user is logged in on Firebase, and display a warning message immediately after login if not.
          await Services().firebase.loginFirebaseEmail(
            email: userEmail,
            password: password,
          );
        } else {
      ...
      await _saveUser(user);
      Analytics.triggerLogin(method: 'password');
      success(user!);
      loading = false;
      notifyListeners();
    } catch (err) {
