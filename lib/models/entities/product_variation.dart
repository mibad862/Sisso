...
class ProductVariation {
  ...
  ///
  /// If not actively on sale, return regular price
  ///
  ...
  double get valuePrice {
    ...
    final vRegularPrice = double.tryParse(regularPrice.toString());
    if (vRegularPrice != null && vRegularPrice > 0) {
      return vRegularPrice;
    }

    return double.tryParse(price.toString()) ?? 0.0;
  }

  /// Returns true when this variation is actively on sale
  ...
  ProductVariation.fromJson(Map parsedJson) {
    ...
    parsedJson['attributes']?.forEach((item) {
      ...
      list.add(attribute);
      attributeMap[attribute.keyAttr] = attribute;
    });
    attributes = list;
  ...
  ProductVariation.fromBigCommerceJson(Map json) {
    ...
    price = json['price']?.toString() ?? json['calculated_price']?.toString();
  ...
  ProductVariation.fromServerlessJson(Map json) {
    id = json['Id']?.toString();
    productId = json['ProductId']?.toString();
    sku = json['SKU']?.toString();
    price = json['Price']?.toString();
    regularPrice =
        json['RegularPrice']?.toString() ?? json['Price']?.toString();
    salePrice = json['SalePrice']?.toString();
    stockQuantity = int.tryParse(json['Stock'].toString());
    inStock = stockQuantity == null || stockQuantity! > 0;
    imageFeature = json['Image'];
    onSale = salePrice != null && (double.tryParse(salePrice!) ?? 0.0) > 0;
    var list = <Attribute>[];
    final options = json['AttributeValues'];
    if (options is Map) {
      for (var option in options.entries) {
        final attribute = Attribute.fromServerlessJson({
          'Name': option.key,
          'Value': option.value,
        });
    ...
    // Parse attribute list if available
    if (json['Attributes'] is List) {
      attributeList = (json['Attributes'] as List)
          .map((item) => VariableAttribute.fromMap(item))
          .toList();
    }
  }
  ...
  ProductVariation.fromMagentoJson(
    ...
    Product product,
    Map<String, ProductAttribute>? allAttributesMap,
  ) {
    String? getCustomAttribute(List? customAttributes, String? attribute) {
      String? value;
      if (customAttributes != null && customAttributes.isNotEmpty) {
        for (var item in customAttributes) {
          if (item is Map && item['attribute_code'] == attribute) {
    ...
    if (dateSaleFrom != null || dateSaleTo != null) {
      ...
      if (dateSaleFrom != null && dateSaleTo != null) {
        onSale =
            now.isAfter(DateTime.parse(dateSaleFrom)) &&
    ...
    salePrice = onSale! ? (pSalePrice ?? price) : price;

    inStock = parsedJson['status'] == 1;

    final imageUrl = getCustomAttribute(
      parsedJson['custom_attributes'],
      'image',
    );
    ...
    for (var element in product.attributes ?? <ProductAttribute>[]) {
      final attributeCode = element.keyAttr;
      final item = getCustomAttribute(
        parsedJson['custom_attributes'],
        attributeCode,
      );
      if (item != null) {
        var productAttribute = allAttributesMap?[attributeCode];
  ...
  ProductVariation.fromShopifyJson(Map parsedJson) {
    ...
    id = parsedJson['id'];
    productId = parsedJson['product']?['id'];
  ...
  ProductVariation.fromPrestaJson(Map json) {
    ...
    final regularPriceValue =
        double.tryParse('${json['regular_price']}') ?? 0.0;
    final salePriceValue = double.tryParse('${json['sale_price']}') ?? 0.0;
    regularPrice = regularPriceValue.toStringAsFixed(2);
    salePrice = salePriceValue.toStringAsFixed(2);
    onSale = salePriceValue > 0 && salePriceValue < regularPriceValue;
    price = (onSale ?? false) ? salePrice : regularPrice;

    stockStatus = json['stock_status']?.toString();
    stockQuantity = int.tryParse('${json['stock_quantity']}');
    final inStockValue = json['in_stock'];
    inStock =
        inStockValue == true ||
    ...
    if (imageUrl?.isNotEmpty == true) {
      ...
      imageFeature = imageUrls.firstOrNull;
    }
    ...
    if (associations != null) {
      ...
      if (productOptionValues is List && productOptionValues.isNotEmpty) {
        for (var item in productOptionValues) {
          final attribute = Attribute.fromPrestaJson(item);
          attributeList.add(attribute);
          attributeMap[attribute.keyAttr] = attribute;
        }
      }
  ...
  ProductVariation.fromHaravanJson(Map json, Map product) {
    id = json['id'].toString();
    final regularPriceItem = (double.parse(
      (json['compare_at_price'] ?? 0.0).toString(),
    ));
    ...
    if (imgId != null) {
      ...
      if (images != null && images is List) {
        final img = images.firstWhereOrNull(
          (element) => element['id'] == imgId,
        );
        if (img != null) {
  ...
  /// Convert ProductVariation to Firestore Map
  Map<String, dynamic> toFirestoreMap() {
    return {
      ...
      'AttributeValues': attributes.fold<Map<String, String>>(
        {},
        (map, attribute) =>
            map
              ..putIfAbsent(attribute.name ?? '', () => attribute.option ?? ''),
      ),
    };
  }
}
