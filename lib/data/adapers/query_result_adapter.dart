...
class QueryResultAdapter extends TypeAdapter<QueryResult> {
  @override
  final int typeId = 100; // Unique ID for this adapter (0-223 reserved for Hive)
  ...
  QueryResult read(BinaryReader reader) {
    try {
      ...
      final data = fields[0] is Map
          ? Map<String, dynamic>.from(fields[0])
          : null;
      final hasException = fields[1] as bool? ?? false;
      final errorMessage = fields[2]?.toString();
      final timestamp =
          fields[3] as int? ?? DateTime.now().millisecondsSinceEpoch;
      final sourceName = fields[4]?.toString();
