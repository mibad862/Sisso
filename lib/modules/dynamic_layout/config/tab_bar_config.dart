...
enum IndicatorStyle { material, dot, rectangular, none }
...
class TabBarConfig {
  ...
  /// Render the tab bar floating above the page content: the area around the
  /// rounded bar becomes transparent and the body extends behind it so the
  /// content underneath shows through. Combine with margins, radius and
  /// boxShadow to get a floating "pill". Unlike [showFloating], this does not
  /// add a center floating action button.
  bool floating = false;

  /// Apply an iOS 26-style liquid glass effect to the tab bar background.
  /// The full refraction shows on Impeller (Android/iOS/macOS); other
  /// platforms gracefully fall back to a frosted look.
  ...
  TabBarConfig.fromJson(dynamic json) {
    ...
    liquidGlass = json['liquidGlass'] ?? false;
