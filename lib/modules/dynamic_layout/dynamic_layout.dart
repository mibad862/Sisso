...
import 'brand/brand_carousel_layout.dart';
...
import 'image_compare/image_compare_layout.dart';
    ...
    switch (config['layout']) {
      case Layout.logo:
        ...
        return LogoWidget(
          ...
          totalCart: Provider.of<CartModel>(
            context,
            listen: true,
          ).totalCartQuantity,
          notificationCount: Provider.of<NotificationModel>(
            context,
          ).unreadCount,
          onSearch: () {
      ...
      case Layout.listCard:
        final configValue = ListCardConfig.fromJson(
      ...
      case Layout.menuList:
        final configValue = MenuListLayoutConfig.fromJson(
          Map<String, dynamic>.from(config),
        );
      ...
      case Layout.category:
        if (config['type'] == 'image') {
          return LayoutLimitWidthScreen(
            child: CategoryImages(config: CategoryConfig.fromJson(config)),
          );
        }

        void onShowProductList(
          CategoryConfig configCategory,
        ...
        if (config['type'] == 'twoRow') {
          ...
          return CategoryTwoRow(
            ...
            onShowProductList: (CategoryItemConfig item) => onShowProductList(
              categoryConfig,
              updateCategoryItemConfig(item, categoryConfig),
            ),
          );
        }
        ...
        return LayoutLimitWidthScreen(
          child: Selector<CategoryModel, Map<String?, Category>>(
            ...
            builder: (context, categoryList, child) {
              ...
              var listCategoryName = categoryList.map(
              ...
              void onTapShowProductList(CategoryItemConfig item) {
                onShowProductList(
                  configValue,
                  updateCategoryItemConfig(item, configValue),
                );
              }
          ...
          return BannerSlider(
            ...
            onTap: (itemConfig) {
              if (itemConfig is Map && itemConfig['category'] != null) {
                final categoryList = Provider.of<CategoryModel>(
                  context,
                  listen: false,
                ).categoryList;
                final id = itemConfig['category'].toString();
                itemConfig['name'] = categoryList[id] != null
      ...
      case Layout.bannerGrid:
        return BannerGrid(config: BannerGridConfig.fromJson(config));
      ...
      case Layout.video:
        return VideoLayout(config: VideoLayoutConfig.fromJson(config));

      case Layout.story:
        return StoryWidget(config: config);
      ...
      case Layout.bannerSlider:
        return Services().widget.renderHorizontalListItem(
          config,
      ...
      case Layout.brand:
        return BrandLayout(config: BrandConfig.fromJson(config));

      case Layout.brandCarousel:
        return BrandCarouselLayout(
          config: BrandCarouselConfig.fromJson(config),
        );
      ...
      case Layout.instagramStory:
        return InstagramStory(config: InstagramStoryConfig.fromJson(config));
      case Layout.tiktokVideos:
        if (ServerConfig().isBuilder || !isMobile) {
          return TikTokVideosPlaceholder();
        }
        return TikTokVideos(config: TikTokVideosConfig.fromJson(config));
      case Layout.webEmbed:
        return WebEmbedLayout(config: WebEmbedConfig.fromJson(config));
      case Layout.imageCompare:
        return ImageCompareLayout(config: ImageCompareConfig.fromJson(config));
