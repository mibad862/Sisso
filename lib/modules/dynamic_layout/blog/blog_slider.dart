...
class BlogSliderWidget extends StatefulWidget {
  const BlogSliderWidget({required this.config, this.blogs, super.key});
...
class _BlogSliderWidgetState extends State<BlogSliderWidget>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.endOfFrame.then((_) async {
      _blogs ??= await _service.api.fetchBlogLayout(config: configJson);
      if (mounted) {
        setState(() {});
      }
    });
  ...
  Widget build(BuildContext context) {
    ...
    return SliderListWidget(
      ...
      onTapSeeAll: () => FluxNavigate.pushNamed(
        RouteList.backdrop,
        arguments: BackDropArguments(config: configJson, data: null),
        context: context,
      ),
      showSubtitleImage: true,
      onTapItem: (context, item) =>
          onTapBlog(blog: item.data, blogs: _blogs, context: context),
      items: _blogs?.map((e) {
