...
class _VerticalViewLayoutWithFilterState
  ...
  void _getAllCategory() async {
    var categorieList = Provider.of<CategoryModel>(
      context,
      listen: false,
    ).categories;
    categories = <Category>[];
    ...
    if (categorieList == null) {
      ...
      categorieList = Provider.of<CategoryModel>(
        context,
        listen: false,
      ).categories;
      if (categorieList == null) {
  ...
  Future<void> _loadProduct({bool forceLoad = false}) async {
    ...
    if (newProducts != null && newProducts.isNotEmpty) {
      ...
      final productLimit = widget.config.limit;
      if (productLimit != null &&
          productLimit > 0 &&
          allProducts.length >= productLimit) {
        allProducts = allProducts.sublist(0, productLimit);
        canLoad.value = false;
      }
  ...
  Widget build(BuildContext context) {
    ...
    final colorText = Theme.of(
      context,
    ).textTheme.bodySmall?.color?.withValueOpacity(0.7);
                        ...
                        children: List.generate(categories?.length ?? 0, (
                          ...
                          return Padding(
                            ...
                            child: GestureDetector(
                              onTap: () => _onSelectCategory(index, category),
                              child: Container(
