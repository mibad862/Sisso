...
class _StateMenuLayout extends State<MenuLayoutWithCustomCategory> {
  final _productCache = <String, ListProductModel>{};
  ...
  Widget renderMenuHeader(bool isFirstLoad, List<Category> categories) {
          ...
          boxShadow: [
            if (categoryConfig.shadow != null)
              BoxShadow(
                ...
                offset: Offset(
                  0,
                  Helper.formatDouble(categoryConfig.shadow ?? 10.0)!,
                ),
              ),
          ],
        ),
                    ...
                    if (widget.config.name != null)
                      SliverToBoxAdapter(
                        child: HeaderView(
                          ...
                          showSeeAll: widget.config.showSeeAll,
                      ...
                      ValueListenableBuilder(
                        ...
                        builder: (context, position, child) {
                          ...
                          return SliverListProductItemWidget(
                            key: ValueKey(id),
                            model: _productCache[id],
                            categoryId: id,
                            putIfAbsent: (value) => _productCache[id] = value,
...
class StateSliverListProductItemWidget
  ...
  SliverMainAxisGroup _buildProductGroup({
    required Widget sliverProducts,
    required Widget loadMoreSliver,
  }) {
    return SliverMainAxisGroup(slivers: [sliverProducts, loadMoreSliver]);
  }

  Widget _buildLoadMoreSliver(ListProductModel model) {
    return SliverToBoxAdapter(
      child: model.hasNext
          ? VisibilityDetector(
  ...
  void initState() {
    ...
    model =
        widget.model ??
        ListProductModel(categoryId: widget.categoryId, limit: limit);
    ...
    return ListenableBuilder(
      ...
      builder: (context, child) {
        if (model.isFirstLoad) {
          return SliverToBoxAdapter(child: kLoadingWidget(context));
        }
        ...
        if (widget.config.cardDesign == CardDesign.horizontal) {
          return _buildProductGroup(
            ...
            loadMoreSliver: _buildLoadMoreSliver(model),
          );
        }

        return _buildProductGroup(
          sliverProducts: SliverPadding(
