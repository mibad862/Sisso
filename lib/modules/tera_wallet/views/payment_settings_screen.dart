...
class _PaymentSettingsScreenState extends State<PaymentSettingsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.endOfFrame.then((_) {
      if (!mounted) return;
      final model = context.read<WalletRequestModel>();
      if (model.paymentMethods?.isEmpty ?? true) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<WalletRequestModel>(
        builder: (context, model, _) {
          final paymentMethods = model.paymentMethods;
          if (model.isPaymentLoading || paymentMethods == null) {
            return const Center(child: CircularProgressIndicator());
          }
          if (paymentMethods.isEmpty) {
            return Center(child: Text(S.of(context).noData));
          }
          _selected ??= paymentMethods.first;
...
class DynamicSettingFields extends StatefulWidget {
  const DynamicSettingFields({super.key, required this.paymentMethod});
...
class _DynamicSettingFieldsState extends State<DynamicSettingFields> {
  ...
  void _submitSettings() {
    ...
    final walletRequestModel = Provider.of<WalletRequestModel>(
      context,
      listen: false,
    );
    walletRequestModel
