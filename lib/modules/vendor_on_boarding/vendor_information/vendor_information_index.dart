...
class VendorIndex extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<VendorOnBoardingModel>(
      ...
      child: VendorInformation(onFinish: onFinish),
    );
  }
}
...
class _VendorInformationState extends State<VendorInformation> {
  ...
  void _updatePage({bool isPrev = false}) {
    ...
    if (!isPrev) {
      if (_currentIndex >= _listPages!.length - 1) {
        final model = Provider.of<VendorOnBoardingModel>(
          context,
          listen: false,
        );
  ...
  Future<void> _updateProfile(VendorOnBoardingModel model) async {
    _onStartUpdating();
    ...
    if (result == ErrorType.updateSuccess) {
      _onEndUpdating(_onFinished);
      return;
    }

    _onUpdateFailed(result);
  }

  void _onUpdateFailed(ErrorType errorType) {
    if (_riveController != null) {
      _artboard.removeController(_riveController!);
    }
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text(errorType.getMessage(context))));
  }
              ...
              children: [
                ...
                Align(
                  ...
                  child: SizedBox(
                    ...
                    child: Stack(
                      children: [
                        Center(child: Text(_currentSlide.toString())),
                        Center(
