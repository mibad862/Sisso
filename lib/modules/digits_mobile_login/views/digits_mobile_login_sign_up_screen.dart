...
class _RegistrationScreenState extends State<DigitsMobileLoginSignUpScreen>
  ...
  @override
  bool get showEmailWhenRegister => kAuthConfig.registration.email.show;
  @override
  bool get showUsernameWhenRegister => kAuthConfig.registration.username.show;
  ...
  void initState() {
    ...
    final defaultCountry = kAuthSmsDefaultCountry;
    if (defaultCountry.dialCode.isNotEmpty ||
        defaultCountry.countryCode.isNotEmpty ||
        defaultCountry.name.isNotEmpty) {
      countryCode = CountryCode(
        code: defaultCountry.countryCode.isNotEmpty
            ? defaultCountry.countryCode
            : null,
        dialCode: defaultCountry.dialCode.isNotEmpty
            ? defaultCountry.dialCode
            : null,
        name: defaultCountry.name.isNotEmpty ? defaultCountry.name : null,
      );
    }
  }
  ...
  bool validateInputs() {
    final isInvalidFirstName =
        requireFirstNameWhenRegister && (firstName?.isEmpty ?? true);
    final isInvalidLastName =
        requireLastNameWhenRegister && (lastName?.isEmpty ?? true);
    final isInvalidEmail = requireEmailWhenRegister && (email?.isEmpty ?? true);
    ...
    if ((mobile?.isEmpty ?? true) ||
      ...
      return false;
    } else if (email != null && email!.isNotEmpty && !email.validateEmail()) {
  ...
  Future<void> _sendSMS(context) async {
    ...
    if (isValid) {
      ...
      try {
        ...
        if (kAuthConfig.enableDigitsMobileFirebase &&
            !kAuthConfig.enableDigitsMobileWhatsApp) {
          ...
          if (sent) {
            ...
            _snackBar(S.of(context).failedToSendOtp);
          }
        }
      } catch (e) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    if (isVendorRegister && isDeliveryRegister && isOwnerRegister) {
      registerText = S.of(context).becomeDifferentRole;
    } else if (isDeliveryRegister) {
      registerText = S.of(context).becomeADelivery;
    } else if (isVendorRegister) {
      registerText = S.of(context).becomeAVendor;
    } else if (isOwnerRegister) {
      registerText = S.of(context).becomeAnOwner;
    }
                          ...
                          children: <Widget>[
                            ...
                            if (showFirstNameWhenRegister) ...[
                              CustomTextField(
                                ...
                                nextNode: showLastNameWhenRegister
                                    ? lastNameNode
                                    : showUsernameWhenRegister
                                    ? usernameNode
                                    : showEmailWhenRegister
                                    ? emailNode
                                    : mobileNode,
                                showCancelIcon: true,
                            ...
                            if (showLastNameWhenRegister) ...[
                              CustomTextField(
                                ...
                                nextNode: showUsernameWhenRegister
                                    ? usernameNode
                                    : showEmailWhenRegister
                                    ? emailNode
                                    : mobileNode,
                            ...
                            if (showUsernameWhenRegister) ...[
                              CustomTextField(
                                ...
                                nextNode: showEmailWhenRegister
                                    ? emailNode
                                    : mobileNode,
                                showCancelIcon: true,
