...
class DigitsMobileLoginServices {
  ...
  Future<User> signUp({
    ...
    try {
      var bodyReq = {
        ...
        'country_code': countryCode,
        'mobile': mobile,
        'whatsapp': kAuthConfig.enableDigitsMobileWhatsApp,
  ...
  Future<bool> loginCheck({
    ...
    try {
      var response = await _postRequest('login/check', {
        'country_code': countryCode,
        'mobile': mobile,
      });
      ...
      if (response is Map && isNotBlank(response['message'])) {
        ...
        switch (code) {
          ...
          case 'invalid_mobile':
            throw Exception(S.current.mobileIsRequired);
          case 'not_existed_mobile':
            throw Exception(S.current.mobileNumberIsNotRegistered);
          default:
            throw Exception(response['message']);
        }
      } else {
