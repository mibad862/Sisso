...
class ShopifyReOrderItemList extends StatefulWidget {
  ...
  const ShopifyReOrderItemList({super.key, required this.lineItems});
  ...
  Future<void> _initializeProducts() async {
    ...
    if (isShopify) {
      ...
      try {
        ...
        _products = await fetchShopifyProductDetailsWithProgress(_products, (
          message,
        ) {
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: Theme.of(
                context,
              ).colorScheme.outline.withValueOpacity(0.2),
            ),
          ),
          child: Column(
            children: [
  ...
  Widget _buildPriceDisplay(BuildContext context, ProductItem productItem) {
    ...
    // 2. Fallback to variant price if available
      ...
      priceString = PriceTools.getVariantPriceProductValue(
        ...
        currencyRate,
        currency,
        onSale: selectedVariation.isOnSaleActive,
      );
    }
