...
class _ReOrderItemListState extends State<ReOrderItemList> {
  ...
  @override
  void initState() {
    printLog('ReOrderItemList: Starting initialization...');
    printLog('ReOrderItemList: LineItems count: ${widget.lineItems.length}');

    for (var item in widget.lineItems) {
      final id = item.id;
      final productId = item.productId;
      printLog(
        'ReOrderItemList: Processing item - id: $id, name: ${item.name}, productId: $productId',
      );

      // Use productId as fallback if id is null (common in Shopify)
      final key = id ?? productId;
      if (key == null) {
        printLog('ReOrderItemList: Skipping item with null id and productId');
        continue;
      }

      printLog('ReOrderItemList: Adding item with key: $key');
      _products[key] = item;
    }

    printLog('ReOrderItemList: Final products count: ${_products.length}');
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return LoadingBody(
      ...
      backgroundColor: Theme.of(
        context,
      ).colorScheme.surface.withValueOpacity(0.4),
      child: Stack(
        children: [
          Align(
                  ...
                  if (_products[_currentProductId]?.product?.type ==
                    ...
                    Padding(
                      ...
                      child: CustomScrollView(
                        slivers: [
                          if (_products[_currentProductId]?.product != null)
      ...
      if (product?.addOns != null &&
        ...
        for (var option in addonsOptions.keys) {
          ...
          for (var addon in product.addOns!) {
            ...
            if (index == -1) {
              final optionIndex = addon.options?.indexWhere(
                (element) =>
                    (element.label != null &&
                ...
                if (addon.name != null && option.contains(addon.name!)) {
                  ...
                  if ((addon.options == null || addon.options!.isEmpty) &&
                    ...
                    final addonsOption = AddonsOption.fromJson({
                      ...
                      'price':
                          double.tryParse(
                            addonsOptions[option]!.replaceAll(
                              RegExp('[^\\d.,]+'),
                              '',
                            ),
                          ) ??
  ...
  Future<void> _addToCart() async {
    ...
    for (var id in _products.keys) {
      ...
      if (product == null) {
        printLog(
          'ReOrderItemList: Skipping product with null product object: $id',
        );
        continue;
      }
      ...
      if (widget.b2bKingIsB2BOrder == true) {
        selectedTieredPrice = B2BKingTieredPrice(
          quantity: 5,
          price:
              double.parse((productItem.total ?? '0')) /
      ...
      if (product.isVariableProduct) {
        ...
        variation = await Services().api.getVariationProduct(
          product.id,
          productItem.variationId,
        );

        for (var item in product.attributes ?? <ProductAttribute>[]) {
          for (var option in item.options ?? []) {
            if (addonsOptions.firstWhereOrNull(
                  (e) => e.toLowerCase() == option['name'].toLowerCase().trim(),
                ) !=
                null) {
              if (item.name == null) continue;
              options[item.name!] = option['name'];
              variation!.attributes.add(
                Attribute(id: item.id, name: item.slug, option: option['slug']),
              );
              break;
            }
          }
